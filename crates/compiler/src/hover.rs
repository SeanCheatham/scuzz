...
pub(crate) const KIT_SIGS: &[(&str, &str)] = &[
    ...
    (
        ...
        "Timeline.signalStrHas(t: Timeline, i: Int, id: Int, needle: String): Bool",
    ),
    (
        "Timeline.a11yHas",
    ...
    ("Fs.list", "Fs.list(path: String): IO[List[(String, Bool)]]"),
    ("Fs.exists", "Fs.exists(path: String): IO[Bool]"),
    ("Fs.delete", "Fs.delete(path: String): IO[Unit]"),
    ("Fs.rename", "Fs.rename(from: String, to: String): IO[Unit]"),
    ("Fs.walk", "Fs.walk(path: String): IO[List[(String, Bool)]]"),
    ("Fs.join", "Fs.join(a: String, b: String): String"),
    ("Fs.dirname", "Fs.dirname(path: String): String"),
    ("Fs.basename", "Fs.basename(path: String): String"),
    ...
    ("Json.pairs", "Json.pairs(j: Json): List[(String, Json)]"),
    ...
    ("Json.floatOr", "Json.floatOr(j: Json, d: Float): Float"),
    (
        "Json.getFloat",
        "Json.getFloat(j: Json, key: String, d: Float): Float",
    ),
    ("Json.set", "Json.set(j: Json, key: String, v: Json): Json"),
    ("Json.remove", "Json.remove(j: Json, key: String): Json"),
    ("Json.append", "Json.append(j: Json, v: Json): Json"),
    ("Json.prepend", "Json.prepend(j: Json, v: Json): Json"),
    ("Json.setAt", "Json.setAt(j: Json, i: Int, v: Json): Json"),
    ("Json.dropAt", "Json.dropAt(j: Json, i: Int): Json"),
    ...
    (
        "Sys.childWrite",
        "Sys.childWrite(pid: Int, s: String): IO[Unit]",
    ),
    (
        "Sys.childRead",
        "Sys.childRead(pid: Int, n: Int): IO[String]",
    ),
    ("Sys.childClose", "Sys.childClose(pid: Int): IO[Unit]"),
...
"#;
    ...
    fn hovers_added_kit_sigs() {
        ...
        let plain_calls = [
            ...
            ("Fs.list", "Fs.list(\".\")"),
            ("Fs.exists", "Fs.exists(\"a\")"),
            ("Fs.delete", "Fs.delete(\"a\")"),
            ("Fs.rename", "Fs.rename(\"a\", \"b\")"),
            ("Fs.walk", "Fs.walk(\".\")"),
            ("Fs.join", "Fs.join(\"a\", \"b\")"),
            ("Fs.dirname", "Fs.dirname(\"a/b\")"),
            ("Fs.basename", "Fs.basename(\"a/b\")"),
            ...
            ("Json.floatOr", "Json.floatOr(Json.Null, 0.0)"),
            ("Json.getFloat", "Json.getFloat(Json.Null, \"a\", 0.0)"),
            ("Json.set", "Json.set(Json.Null, \"a\", Json.Null)"),
            ("Json.remove", "Json.remove(Json.Null, \"a\")"),
            ("Json.append", "Json.append(Json.Null, Json.Null)"),
            ("Json.prepend", "Json.prepend(Json.Null, Json.Null)"),
            ("Json.setAt", "Json.setAt(Json.Null, 0, Json.Null)"),
            ("Json.dropAt", "Json.dropAt(Json.Null, 0)"),
            ("Sys.read", "Sys.read(4)"),
            ("Sys.write", "Sys.write(\"x\")"),
            ("Sys.exec", "Sys.exec(\"ls\")"),
            ("Sys.spawn", "Sys.spawn(\"ls\")"),
            ("Sys.childWrite", "Sys.childWrite(1, \"x\")"),
            ("Sys.childRead", "Sys.childRead(1, 4)"),
            ("Sys.childClose", "Sys.childClose(1)"),
