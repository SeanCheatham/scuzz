...
int main(void) {
  ...
  /* List.zip / zipAll / unzip / transpose: align lists as packed pairs. */
  {
    size_t base_bytes = 0, base_count = 0;
    size_t live_bytes = 0, live_count = 0;
    SzString *a;
    ...
    pair = (SzList *)sz_list_head(zs);
    assert(sz_list_len(pair) == 2);
    assert(pair->head == a);
    assert(sz_list_at(pair, 1) == one);
    pair = (SzList *)sz_list_at(zs, 1);
    assert(pair->head == b);
    assert(sz_list_at(pair, 1) == two);
    sz_list_free(zs);
    ...
    pair = (SzList *)sz_list_at(zs, 2);
    assert(pair->head == c);
    assert(sz_list_at(pair, 1) == nine);
    sz_list_free(zs);
    zs = sz_list_zip_all(ys, xs, zee, nine);
    assert(sz_list_len(zs) == 3);
    pair = (SzList *)sz_list_at(zs, 2);
    assert(pair->head == zee);
    assert(sz_list_at(pair, 1) == c);
    sz_list_free(zs);
    zs = sz_list_zip_all(NULL, NULL, zee, nine);
    assert(sz_list_is_empty(zs));
    zs = sz_list_zip(xs, ys);
    tmp = sz_list_unzip(zs);
    sz_list_free(zs);
    assert(sz_list_len(tmp) == 2);
    g0 = (SzList *)sz_list_head(tmp);
    g1 = (SzList *)sz_list_at(tmp, 1);
    assert(sz_list_len(g0) == 2);
    assert(g0->head == a);
    ...
    sz_list_free(tmp);
    tmp = sz_list_unzip(NULL);
    assert(sz_list_len(tmp) == 2);
    assert(sz_list_is_empty((SzList *)sz_list_head(tmp)));
    assert(sz_list_is_empty((SzList *)sz_list_at(tmp, 1)));
    sz_list_free(tmp);
    pair = sz_list_cons(a, NULL);
    zs = sz_list_cons(pair, NULL);
    sz_release(pair);
    tmp = sz_list_unzip(zs);
    sz_list_free(zs);
    assert(sz_list_len(tmp) == 2);
  ...
  /* List.contains / indexOf / lastIndexOf / distinct / diff / intersect. */
  {
    size_t base_bytes = 0, base_count = 0;
    size_t live_bytes = 0, live_count = 0;
    SzString *a;
