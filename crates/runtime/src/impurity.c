...
static SzIo *after_line(void *value, void *env) {
  (void)env;
  return sz_io_flatmap(labeled("line:", (SzString *)value), kit_done, NULL);
}

static SzIo *do_read_line(void *value, void *env) {
  (void)value;
  (void)env;
  return sz_io_flatmap(sz_sys_read_line(), after_line, NULL);
}
...
static SzIo *after_args(void *value, void *env) {
  ...
  return sz_io_flatmap(labeled("args:", joined), do_read_line, NULL);
}

static SzIo *do_args(void *value, void *env) {
  (void)value;
  (void)env;
  return sz_io_flatmap(sz_sys_args(), after_args, NULL);
}

static SzIo *after_net(void *value, void *env) {
  (void)env;
  return sz_io_flatmap(labeled("net:", (SzString *)value), do_args, NULL);
}
...
static SzIo *after_fs(void *value, void *env) {
  (void)env;
  return sz_io_flatmap(labeled("fs:", (SzString *)value), do_net, NULL);
}

static SzIo *do_fs_read(void *value, void *env) {
  (void)value;
  (void)env;
  return sz_io_flatmap(sz_fs_read(sz_string_from_cstr("note.txt")), after_fs,
                       NULL);
}
...
static SzIo *after_rand(void *value, void *env) {
  (void)env;
  return sz_io_flatmap(label_i64("rand:", sz_unbox_i64(value)), do_fs_write,
                       NULL);
}
...
static SzIo *do_rand(void *value, void *env) {
  (void)value;
  (void)env;
  return sz_io_flatmap(sz_random_next_int(100), after_rand, NULL);
}
...
static SzIo *do_sleep(void *value, void *env) {
  (void)value;
  (void)env;
  return sz_io_flatmap(sz_io_sleep_ms(50), after_sleep, NULL);
}

static SzIo *after_mono(void *value, void *env) {
  (void)env;
  return sz_io_flatmap(label_i64("mono:", sz_unbox_i64(value)), do_sleep, NULL);
}

static SzIo *do_mono(void *value, void *env) {
  (void)value;
  (void)env;
  return sz_io_flatmap(sz_clock_monotonic(), after_mono, NULL);
}

static SzIo *after_real(void *value, void *env) {
  (void)env;
  return sz_io_flatmap(label_i64("real:", sz_unbox_i64(value)), do_mono, NULL);
}
...
static SzIo *impurity_boot(void *value, void *env) {
  ...
  return sz_io_flatmap(sz_clock_real_time(), after_real, NULL);
}

SzIo *sz_impurity_run_kit(void) {
  return sz_io_flatmap(sz_io_delay(impurity_unit_thunk, NULL), impurity_boot, NULL);
}
