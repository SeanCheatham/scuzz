...
- Builtin `IO` + blessed kit (`Resource`, `Stream`, `Ref`, `Deferred`, `Queue`, `Fiber.fork`/`join`/`interrupt`, race/both/timeout/forever/repeatN/retryN/sleep/errors)
- Blessed impurity: `Clock` / `Random` / `Fs` / `Net` / `Sys` / `IO.println` + hermetic `TestRuntime` fakes (`Sys.exec` / `Sys.spawn` / child stdio fail under TestRuntime)
- `Ui` session (Headless / Desktop / Mobile peers): View tree, signals, theme, a11y hooks
