...
fn watch_run(path: &Path, out_dir: &Path, headless: bool) -> Result<ExitCode> {
    ...
    eprintln!(
        "scuzz run --watch {} (hot reload: recompiles build/reload.dylib then stamp-reloads View tree; live dump build/debug.dump; inject build/inject.script including dump/reload/quit/resetpeak/key/caret)",
        project_dir.display()
    );
