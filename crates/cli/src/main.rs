...
enum Commands {
    ...
    Build {
        ...
        #[arg(long)]
        full: bool,
        /// Apply `*.scuzz_sim` / `*.scuzz_drivers` / `*.scuzz_verify` + residual `.require` / `where` (TestRuntime / fuzz graph)
        #[arg(long)]
        verify: bool,
    },
