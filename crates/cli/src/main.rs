...
enum Commands {
    ...
    #[command(
        after_help = "Examples:\n  scuzz run\n  scuzz run --headless\n  scuzz run examples/studio\n  scuzz run --headless --script examples/studio/build/record.script --dump examples/studio/build/debug.dump examples/studio\n  scuzz run --watch --headless\n\nDesktop/Mobile `run` writes build/record.script (live OS input) and build/debug.dump. Replay Headless with --script and --dump. `[ui] run --watch` is hot reload. `build/inject.script` stays watch playback only (`dump` / `reload` / `quit` included)."
    )]
    Run {
        #[arg(default_value = ".")]
        path: PathBuf,
...
fn watch_run(path: &Path, out_dir: &Path, headless: bool) -> Result<ExitCode> {
    ...
    eprintln!(
        "scuzz run --watch {} (hot reload: recompiles build/reload.dylib then stamp-reloads View tree; live dump build/debug.dump; inject build/inject.script including dump/reload/quit)",
        project_dir.display()
    );
