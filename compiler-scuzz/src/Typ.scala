...
def callFsCanonicalize(argTys: List): String =
  for {
...
def callSysReadLine(argTys: List): String =
  for {
    a = expectArity("Sys.readLine", 0, List.len(argTys))
  } yield if (tyIsOk(a) == 0) a else tyOk("IO[String]")
