...
def parseIdent(tokens: List, i: Int): List =
  ...
  if (isIdentTok(t) == 1) okStr(identName(t), i + 1) else okStr("", i)
...
def parsePackageParts(tokens: List, i: Int, acc: String): List =
  ...
  if (isTok(tokens, i1, "Dot") == 1) parsePackageParts(tokens, i1 + 1, next) else okStr(next, i1)
...
def parseParamsCont(tokens: List, i: Int, acc: List): List =
  ...
  if (isTok(tokens, pIdx(p), "Comma") == 1) parseParams(tokens, pIdx(p) + 1, acc2) else ok(List.reverse(acc2), pIdx(p))
...
def parseArgsCont(tokens: List, i: Int, acc: List): List =
  ...
  if (isTok(tokens, pIdx(e), "Comma") == 1) parseArgs(tokens, pIdx(e) + 1, acc2) else ok(List.reverse(acc2), pIdx(e))
...
def parseListLitElemsCont(tokens: List, i: Int, acc: List): List =
  val e = parseExpr(tokens, i)
  val acc2 = List.cons(pAst(e), acc)
  if (isTok(tokens, pIdx(e), "Comma") == 1) parseListLitElems(tokens, pIdx(e) + 1, acc2) else parseListLitElemsEnd(tokens, pIdx(e), acc2)
...
def parseBlock(tokens: List, i: Int): List =
  if (isTok(tokens, i, "Val") == 1) parseLet(tokens, i + 1) else parseBlockExpr(tokens, i)

def parseBlockExpr(tokens: List, i: Int): List =
  val e = parseExpr(tokens, i)
  if (isTok(tokens, pIdx(e), "Val") == 1) parseBlockStmt(tokens, pAst(e), pIdx(e)) else e

def parseBlockStmt(tokens: List, stmt: List, i: Int): List =
  val bodyP = parseBlock(tokens, i)
  ok(List.cons("Let", List.cons("_", List.cons(stmt, List.cons(pAst(bodyP), List.empty())))), pIdx(bodyP))
...
def parseLet(tokens: List, i: Int): List =
  ...
  ok(List.cons("Let", List.cons(pStr(nameP), List.cons(pAst(valueP), List.cons(pAst(bodyP), List.empty())))), pIdx(bodyP))
...
def parseOrRestOp(tokens: List, left: List, i: Int): List =
  ...
  parseOrRest(tokens, List.cons("BinOp", List.cons("||", List.cons(left, List.cons(pAst(rightP), List.empty())))), pIdx(rightP))
...
def parseAndRestOp(tokens: List, left: List, i: Int): List =
  ...
  parseAndRest(tokens, List.cons("BinOp", List.cons("&&", List.cons(left, List.cons(pAst(rightP), List.empty())))), pIdx(rightP))
...
def parseCmpRest(tokens: List, left: List, i: Int): List =
  ...
  if (Str.len(op) == 0) ok(left, i) else parseCmpRestOp(tokens, left, i + 1, op)

def parseCmpRestOp(tokens: List, left: List, i: Int, op: String): List =
  val rightP = parseAdd(tokens, i)
  parseCmpRest(tokens, List.cons("BinOp", List.cons(op, List.cons(left, List.cons(pAst(rightP), List.empty())))), pIdx(rightP))
...
def parseAddRestOp(tokens: List, left: List, i: Int, op: String): List =
  ...
  parseAddRest(tokens, List.cons("BinOp", List.cons(op, List.cons(left, List.cons(pAst(rightP), List.empty())))), pIdx(rightP))
...
def parseMulRestOp(tokens: List, left: List, i: Int, op: String): List =
  ...
  parseMulRest(tokens, List.cons("BinOp", List.cons(op, List.cons(left, List.cons(pAst(rightP), List.empty())))), pIdx(rightP))
...
def parseMatch(tokens: List, scrut: List, i: Int): List =
  ...
  parsePostfixRest(tokens, List.cons("Match", List.cons(scrut, List.cons(pAst(armsP), List.empty()))), i2)
...
def parseMatchArm(tokens: List, i: Int, acc: List): List =
  ...
  parseMatchArms(tokens, pIdx(bodyP), List.cons(List.cons("Arm", List.cons(pAst(patP), List.cons(pAst(bodyP), List.empty()))), acc))
...
def parsePatternAdt(tokens: List, i: Int): List =
  ...
  ok(List.cons("PatAdt", List.cons(pStr(enumP), List.cons(pStr(caseP), List.empty()))), pIdx(caseP))

def parsePostfixDot(tokens: List, expr: List, i: Int): List =
  val methodP = parseIdent(tokens, i)
  val method = pStr(methodP)
  val i1 = pIdx(methodP)
  if (streq(method, "flatMap") == 1) parseFlatMap(tokens, expr, i1) else if (streq(method, "handleErrorWith") == 1) parseHandle(tokens, expr, i1) else if (streq(method, "attempt") == 1) parseAttempt(tokens, expr, i1) else ok(expr, i1)

def parseLambda(tokens: List, i: Int): List =
  val t = tokAt(tokens, i)
  if (streq(t, "Underscore") == 1) parseLambdaBody(tokens, i + 1, "_") else if (isIdentTok(t) == 1) parseLambdaBody(tokens, i + 1, identName(t)) else if (streq(t, "LParen") == 1) parseLambdaUnit(tokens, i + 1) else ok(unitExpr(), i)
...
def parseFlatMap(tokens: List, expr: List, i: Int): List =
  ...
  parsePostfixRest(tokens, List.cons("FlatMap", List.cons(expr, List.cons(param, List.cons(body, List.empty())))), i2)
...
def parseHandle(tokens: List, expr: List, i: Int): List =
  ...
  val i2 = expectTok(tokens, pIdx(lamP), "RParen")
  val body = List.head(List.tail(pAst(lamP)))
  parsePostfixRest(tokens, List.cons("Handle", List.cons(expr, List.cons(body, List.empty()))), i2)
...
def parseInterpBrace(tokens: List, i: Int, acc: List, body: String): List =
  ...
  parseInterp(tokens, i + 1, List.cons(List.cons("Hole", List.cons(pAst(nested), List.empty())), acc))
...
def parseLambdaExpr(tokens: List, i: Int, param: String): List =
  val i1 = expectTok(tokens, i, "Arrow")
  val bodyP = parseBlock(tokens, i1)
  ok(List.cons("Lambda", List.cons(param, List.cons(pAst(bodyP), List.empty()))), pIdx(bodyP))

def parseNegInt(tokens: List, i: Int): List =
  val t = tokAt(tokens, i)
  if (isIntTok(t) == 1) ok(List.cons("IntLit", List.cons(Str.concat("-", intDigits(t)), List.empty())), i + 1) else ok(unitExpr(), i)
...
def parseIf(tokens: List, i: Int): List =
  ...
  ok(List.cons("If", List.cons(pAst(condP), List.cons(pAst(thenP), List.cons(pAst(elseP), List.empty())))), pIdx(elseP))

def parseIfBranch(tokens: List, i: Int): List =
  if (isTok(tokens, i, "Val") == 1) parseBlock(tokens, i) else parseExpr(tokens, i)
...
def parseDotCallOrAdt(tokens: List, i: Int, enumName: String): List =
  ...
  val j = pIdx(caseP)
  if (isTok(tokens, j, "LParen") == 1) parseCallArgs(tokens, j + 1, str3(enumName, ".", caseName)) else ok(List.cons("Adt", List.cons(enumName, List.cons(caseName, List.empty()))), j)
...
def parseModuleCall(tokens: List, i: Int, mod: String): List =
  ...
  val i2 = pIdx(methodP)
  if (isTok(tokens, i2, "LParen") == 1) parseModuleCallArgs(tokens, i2 + 1, mod, callee) else ok(List.cons("Call", List.cons(callee, List.cons(List.empty(), List.empty()))), i2)
...
def parseIo(tokens: List, i: Int): List =
  ...
  val method = pStr(methodP)
  val i2 = pIdx(methodP)
  if (streq(method, "delay") == 1) parseIoDelay(tokens, i2) else if (isTok(tokens, i2, "LParen") == 1) parseIoArgs(tokens, i2 + 1, Str.concat("IO.", method)) else ok(List.cons("Call", List.cons(Str.concat("IO.", method), List.cons(List.empty(), List.empty()))), i2)
...
def parseIoArgs(tokens: List, i: Int, callee: String): List =
  ...
  val i1 = expectTok(tokens, pIdx(argsP), "RParen")
  val args = pAst(argsP)
  if (streq(callee, "IO.println") == 1) ok(List.cons("Println", List.cons(List.head(args), List.empty())), i1) else if (streq(callee, "IO.fail") == 1) ok(List.cons("Fail", List.cons(List.head(args), List.empty())), i1) else if (streq(callee, "IO.pure") == 1) ok(List.cons("Pure", List.cons(List.head(args), List.empty())), i1) else if (streq(callee, "IO.sleep") == 1) ok(List.cons("Sleep", List.cons(List.head(args), List.empty())), i1) else if (streq(callee, "IO.race") == 1) ok(List.cons("IoRace", List.cons(List.head(args), List.cons(List.head(List.tail(args)), List.empty()))), i1) else if (streq(callee, "IO.both") == 1) ok(List.cons("IoBoth", List.cons(List.head(args), List.cons(List.head(List.tail(args)), List.empty()))), i1) else ok(List.cons("Call", List.cons(callee, List.cons(args, List.empty()))), i1)
...
def parseDef(tokens: List, i: Int): List =
  ...
  ok(List.cons("Def", List.cons(pStr(nameP), List.cons(pAst(paramsP), List.cons(pStr(retP), List.cons(pAst(bodyP), List.empty()))))), pIdx(bodyP))
...
def parseEnum(tokens: List, i: Int): List =
  val nameP = parseIdent(tokens, i)
  val i1 = pIdx(nameP)
  if (isTok(tokens, i1, "LBrace") == 1) parseEnumBrace(tokens, i1 + 1, pStr(nameP), List.empty()) else if (isTok(tokens, i1, "Colon") == 1) parseEnumColon(tokens, i1 + 1, pStr(nameP), List.empty()) else ok(List.cons("Enum", List.cons(pStr(nameP), List.cons(List.empty(), List.empty()))), i1)
...
def parseEnumBraceCase(tokens: List, i: Int, name: String, acc: List): List =
  ...
  if (isTok(tokens, i2, "Comma") == 1) parseEnumBrace(tokens, i2 + 1, name, acc2) else parseEnumBrace(tokens, i2, name, acc2)
