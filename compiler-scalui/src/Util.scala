...
// Heterogeneous pair (String, List) for emit state bundles.
...
def indexOfChar(s: String, ch: Int, i: Int): Int =
  if (i >= Str.len(s)) 0 - 1
  else if (Str.charAt(s, i) == ch) i
  else indexOfChar(s, ch, i + 1)

def lastIndexOfChar(s: String, ch: Int, i: Int, last: Int): Int =
  if (i >= Str.len(s)) last
  else if (Str.charAt(s, i) == ch) lastIndexOfChar(s, ch, i + 1, i)
  else lastIndexOfChar(s, ch, i + 1, last)
...
def parentDirAt(path: String, slash: Int): String =
  ...
  else Str.slice(path, 0, slash)

def asciiPrintable(): String =
  " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~"

def hexDigit(n: Int): String = Str.slice("0123456789ABCDEF", n, n + 1)

def hex2(c: Int): String = str3("\\", hexDigit(c / 16), hexDigit(c - (c / 16) * 16))

def charToStr(c: Int): String =
  if (c >= 32) if (c <= 126) Str.slice(asciiPrintable(), c - 32, c - 31) else hex2(c)
  else hex2(c)
...
def llvmEscapeChar(c: Int): String =
  ...
  else if (c >= 32) if (c <= 126) charToStr(c) else hex2(c)
  else hex2(c)

def llvmEscapeAt(s: String, i: Int, acc: String): String =
  if (i >= Str.len(s)) acc
  else llvmEscapeAt(s, i + 1, Str.concat(acc, llvmEscapeChar(Str.charAt(s, i))))

def llvmEscape(s: String): String = llvmEscapeAt(s, 0, "")

def listConcat(a: List, b: List): List =
  if (List.isEmpty(a) == 1) b
  else List.cons(List.head(a), listConcat(List.tail(a), b))

def nodeStr(e: List, i: Int): String = List.at(List.tail(e), i)
def nodeExpr(e: List, i: Int): List = List.at(List.tail(e), i)
...
def mkS(code: String, value: String, kind: String, id: Int, conts: String): List =
  List.cons(
    ...
    List.cons(
      value,
      List.cons(kind, List.cons(Str.fromInt(id), List.cons(conts, List.empty)))
    )
  )

def sCode(s: List): String = List.head(s)
def sValue(s: List): String = List.head(List.tail(s))
def sKind(s: List): String = List.head(List.tail(List.tail(s)))
def sId(s: List): Int = parseInt(List.head(List.tail(List.tail(List.tail(s)))))
def sConts(s: List): String = List.head(List.tail(List.tail(List.tail(List.tail(s)))))
...
def envGet(env: List, name: String): String =
  if (List.isEmpty(env) == 1) "null:ptr"
  else envGetCont(List.head(env), env, name)

def envGetCont(b: String, env: List, name: String): String =
  if (streq(envBindName(b), name) == 1) envBindRest(b)
  else envGet(List.tail(env), name)

def envGetVal(env: List, name: String): String = envRestVal(envGet(env, name))
def envGetKind(env: List, name: String): String = envRestKind(envGet(env, name))
