...
- **Memory** — Headless pumps stay flat under alloc accounting. Last-use retain/release is locked in [`vision.md`](vision.md) GC. User function calls drop owned pointer arguments and take owned String / List / Map / Set / ADT returns. A discarded `_` let drops an owned pointer or unused IO without sharing a local slot. An unused named IO let drops after the body. Capture packs drop a boxed Int or Float after cons. A tap lambda drops a leftover owned pointer or the run result. `Law.check` / `Law.assert` / `Law.sometimes` / `Law.a11yHas` drop an owned name or needle. `Law.check` keeps the value owned. `Law.signalStr` / `Law.signalListAt` return an owned string. `Law.force` drops the unbox box. Values with no last-use stay allocated. Queue, Ref, Deferred, Either, pair, Fiber.join, IO.pure, IO.fail, and delay env are RC or leftover `sz_alloc`; last-use drops the cell, the payload, the error, leftover RC env, or leftover `sz_alloc` delay env. The live debug dump includes `[session]` (kind / size / lifecycle / keyboard / pumps), `[heap]` (`live_bytes` / `live_count` / `peak_bytes`, `delta_bytes` / `delta_count`, and per-kind `name=count:bytes`), and `[live]` remaining blocks (`kind rc=N bytes=M`, capped) so agents can watch the session and see which RC objects remain. Fuzz / golden dumps omit them. Inject `resetpeak` sets peak to live and marks the delta. Panic prints remaining `[heap]` / `[live]`, writes `SCUZZ_PANIC_DUMP` or `*.panic`, frees remaining live blocks, then abort. No cycle collector.
- **LSP / editor tooling** — `fmt`, `check --message-format=json`, `watch`, and `scuzz lsp` exist. LSP wraps `check` (didOpen/didChange overlay open buffers; didClose returns to disk; `workspace/didChangeWatchedFiles` republishes check). Positions are UTF-16. Language-server methods use that parse. Type hierarchy walks trait, record, enum, and impl links. Unknown `IO.*` methods parse as calls so check reports them and code actions can rename them. `check` reports unused imports, unused locals, unused parameters, and unused private defs. JSON diagnostics stay the single schema. `check` reports more than one parse or type error per run. Method catalog: [`vision.md`](vision.md#tooling).
