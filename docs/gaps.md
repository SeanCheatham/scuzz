...
- **Oracle idioms in `guide.md`** — `After tap` response claims and `never` forms are in. Given rows seed known input/output pairs. The English grammar is frozen. It is the author surface. Sentences lower to timeline predicates. Richer templates will never land: expressiveness comes from that lowering, not grammar growth. Metamorphic swap and concrete-fact `.require` are in.
- **Timeline claim kernel** — the current verification direction. Claims as pure predicates over `Timeline`, judged at a constructed terminal point (quiesce phase). Shipped grammar sentences lower to timeline predicates; the runtime claim latches go away. ∀/∃ quantification over explored timelines. Metamorphic relations stay a distinct pair-of-timelines kind. `.require` stays a live point assertion.
- **Observation-first `State`** — observation section first (signals, a11y, hits, drive records). Effect log (op names, sizes or hashes, never payloads), fiber census, heap deltas, and fault context enter when a universal oracle or claim needs them. Deterministic capture: creation-order ids, no pointers, no hash-map order. Versioned schema.
- **Terminal-point universal oracles** — double-run same-seed determinism, heap returns to baseline, acquire/release pairing, finalizer-on-cancel, no parked fibers at quiescence, live/verify differential equivalence.
- **Coverage axes** — breadth: `scuzz check` reports unclaimed defs, signals, and controls; the campaign reports reached states that vary in `State` fields no claim reads. Strength: the mutation-survivor split (weak claim in a claimed field, missing claim in an unclaimed field; inert mutants filtered by bit-identical replay). Reachability stays `sometimes` plus unified unfired-trigger failure.
