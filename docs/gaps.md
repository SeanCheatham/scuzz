...
2. **Checker and emit residuals** — Param letters (`A`/`E`) still unify. Stream unfold still uses a bare constructor as some T. Resource, Queue, and Deferred still use a bare constructor as some T. Map.empty and Set.empty still return a bare constructor. List.empty still returns a bare `List`. The parser stores Fun/Param types as strings; Check parses them. A path-dep file over 40k keeps def heads with a stub body so Check can resolve a qualified call.
