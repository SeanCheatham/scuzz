...
- **Flutter-style constraint layout** — `layout_node_ex` passes min/max down. Tight slots: `View.sized`, `View.aspectRatio`, percent axes on `View.fraction`, `View.expanded` flex, and opt-in `View.stretch` (cross axis). `View.minSize` raises min. `View.maxSize` lowers max (`0` = no cap). Incoming max still wins when tighter. Residual: Flutter-style constraint-overflow dumps (do not add).
