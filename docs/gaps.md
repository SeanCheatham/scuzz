...
- **Memory** — Headless pumps stay flat under alloc accounting. Last-use retain/release is locked in [`vision.md`](vision.md) GC. Values with no last-use stay allocated. Queue, Ref, Deferred, Either, pair, and Fiber.join results are RC; last-use drops the cell or the join payload. The IO.pure payload still aliases the run result. No cycle collector. Panic may leak.
