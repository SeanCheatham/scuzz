...
- **Memory** — Headless pumps stay flat under alloc accounting. Last-use retain/release is locked in [`vision.md`](vision.md) GC. Values with no last-use stay allocated. Queue, Ref, Deferred, Either, and pair do not release leftover payloads when they free. `Queue.take` still treats the payload as borrowed, so the transferred offer ref leaks. No cycle collector. Panic may leak.
