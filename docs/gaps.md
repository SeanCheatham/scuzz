...
- **Net listen** — `Net.serveOnce` one GET; `Net.serve` keeps the listen socket (Stage 0 and self-host; `examples/server`). TestRuntime queues injected paths and drains them. Live listen, connection read/write, and `httpGet` park on `poll` (`test_io.c`). DNS `getaddrinfo` still blocks (code **6**). Later: poll stdin `readLine`.
