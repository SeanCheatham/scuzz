...
- **Stronger mutation operators** — boundary `where` mutations are in (Oracles mode negates or drops the residual bound). Remaining: handler-swap on UI taps, Signal-map identity. Then a mutation score in `summary.toml`.
