...
1. **Compile-time performance** — `scuzz check examples/compiler` is 59 s on this host. A cold `scuzz build examples/tyck` is 61 s on this host. Re-time both commands before another show-and-parse slice. Emitted string literals intern to pinned allocations. Kit signatures parse to `Ty` when the table is built. Generic kit calls compare those `Ty` values. `zipCheck` and `checkKnownRet` keep `Ty`. Env lookup returns `Ty`. A borrowed tail of a live parameter does not retain. A single-arm match on a borrowed value does not retain a borrowed result. Owned concat reuses a unique left value. A shared or pinned left value is copied. Remaining cost: `sz_list_cons` retains the head and the tail. `concreteTy` still parses a shown type. Coverage uses the live program when compiled files match live.
