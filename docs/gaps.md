...
Then the staged rewrite slices ([`vision.md`](vision.md#self-hosting)): fmt core is in (`examples/fmt`). The kernel typechecker is in (`examples/tyck`). Kernel codegen is in (`examples/codegen`). CLI core is in (`examples/cli`). Driver core is in (`examples/compiler`). Check dispatch is in. Test/fuzz core is in. Hello package compile is in. Fmt-shaped `if` / String / `Str.concat` IR clang-links and prints `fmt-ok`. Int compare, ptr params, nullary enum construct, and nullary `match` IR clang-links and prints `r`. Next is compile the rest of `examples/`, compile itself, and Rust retirement. Each slice is a Scuzz package with roundtrip, idempotence, and differential oracles. The last Rust `scuzz` ships as a tagged GitHub Release and becomes the bootstrap compiler. The Rust crates retire when that tagged binary compiles the Scuzz toolchain, and that toolchain compiles `examples/` and itself.
