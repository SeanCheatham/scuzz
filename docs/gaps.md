...
- **Generators** — Drivers and drive oracles accept generator-friendly parameters. Search varies typed arguments and stored event sequences. It keeps simple Int bounds and bounds ADT depth. Event reduction preserves argument types. Argument reduction remains open. Direction: `Gen[T]` combinators and shrinking that keeps `where` bounds. Stateful model generators stay later.
