...
- **Net listen** — `Net.serveOnce` one GET; `Net.serve` keeps the listen socket (Stage 0 and self-host; `examples/server`). TestRuntime queues injected paths and drains them. Live accept still blocks the fiber like `httpGet` (code **6**). Later: nonblocking poll.
