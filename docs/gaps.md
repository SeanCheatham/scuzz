...
Time, regex, hash, hex/base64, UUID. `Map` / `Set` keys beyond `Int` or `String`. `scuzz eval`. Generators. Drive `==` wrap on UI. OS threads. HTTPS serve with app cert and key files.
