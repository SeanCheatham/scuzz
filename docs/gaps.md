...
- **Studio heap growth under fuzz** — fixed: `Signal.getStr` marked its copy borrowed (leaked per call), and the session baseline ran before unmount, when view-owned capture packs still pin session values. Studio search and heap oracles are green; the mutation phase reports weak/missing claims on the widget-catalog row (next slice).
