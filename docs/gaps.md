...
- **Language surface** — richer generics beyond monomorphized defs/enums/records, record/enum methods, generic `impl`, `trait Get[T]`, and `impl Get[Int] for Point` ([`compatibility.md`](compatibility.md)). Next: explicit trait args on a generic receiver.
