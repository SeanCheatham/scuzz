...
- **Hot reload and debugging tools** — Headless + in-process reload + debug for agents. Headless is a peer; `scuzz watch` only rebuilds; `[ui]` `run --watch` stamp-reloads the View tree (Signals stay), writes `build/debug.dump` (signals + a11y + `[taps]` for `tap N` + `[fields]` for `text`/`type`/`backspace`, including `text N s` / `type N s` / `backspace N k`), and plays `build/inject.script`. Still missing: new machine code in-process. Do not document `watch` as hot reload.
