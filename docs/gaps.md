...
Then the staged rewrite slices ([`vision.md`](vision.md#self-hosting)): fmt core is in (`examples/fmt`). The kernel typechecker is in (`examples/tyck`). Kernel codegen is in (`examples/codegen`). CLI core is in (`examples/cli`). Driver core is in (`examples/compiler`). Next is the verification stack and Rust retirement. Each slice is a Scuzz package with roundtrip, idempotence, and differential oracles. The Rust binary retires when the Scuzz toolchain passes the full campaign, compiles `examples/`, and compiles itself.
