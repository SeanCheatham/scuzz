...
- **Concurrency** — cooperative fibers + TestRuntime virtual-time jumps (`test_io.c`); Scuzz `Ref` / `Queue` / `Deferred` (String payloads) via Stage 0 + self-host (`examples/concurrency`). Live `IO.race` of sleeps waits only for the soonest wake; idle `nanosleep` is EINTR-interruptible so a cancelled sleeper cannot hold the run loop. Later: OS threads, supervision trees.
