...
- **Oracle idioms in `guide.md`** — `After tap` response claims and `never` forms are in. Given rows seed known input/output pairs. The English grammar is frozen. It becomes the specification of the renderable fragment and the renderer's round-trip oracle. Richer templates will never land: expressiveness comes from Scuzz claim predicates, not grammar growth. Metamorphic swap and concrete-fact `.require` are in.
- **Candidate synthesis and the judgment queue** — `scuzz mine` proposes claims from fuzz traces. Direction: synthesis moves into the campaign as a streaming fold over timelines, from four sources (holds, forks, complaints, mutant minimal pairs). `scuzz mine` becomes the queue interface. An agent accepts a candidate by citing an intent line or judgment-log entry that entails it (`--because`). Everything else goes to the human as a forced-choice question with a replay identity. Accepted claims land in `intent.scuzz_intent`. Candidates qualify on evidence, mutation-kill rank, directed probes, and mutation relevance. `scuzz mine --json` feeds an external assistant. ML stays outside `scuzz`. See [`mine.md`](mine.md).
- **Timeline claim kernel** — claims as pure Scuzz predicates over `Timeline`, judged at a constructed terminal point (quiesce phase). ∀/∃ quantification over explored timelines. Metamorphic relations stay a distinct pair-of-timelines kind. `.require` stays a live point assertion.
- **Effect-captured `State`** — observation (signals, a11y, hits), effect log (op names, sizes or hashes, never payloads), fiber census, heap deltas, fault context, drive records. Deterministic capture: creation-order ids, no pointers, no hash-map order. Versioned schema; a field enters when a second claimant needs it.
- **Terminal-point universal oracles** — heap returns to baseline, acquire/release pairing, finalizer-on-cancel, no parked fibers at quiescence, double-run same-seed determinism, live/verify differential equivalence.
- **Coverage axes** — breadth: reached states that vary in `State` fields no claim reads feed the judgment queue. Strength: the mutation-survivor split (weak claim in a claimed field, missing claim in an unclaimed field; inert mutants filtered by bit-identical replay). Reachability stays `sometimes` plus unified unfired-trigger failure.
- **Judgment log** — structured, append-only, machine-written provenance. The intent file is its projection. Hand-written claims record as judgments without questions.
- **Stronger mutation operators** — boundary `where` mutations, handler-swap on UI taps, Signal-map identity. Then a mutation score in `summary.toml`. Operator quality sets question quality for mutant minimal-pair queue items.
