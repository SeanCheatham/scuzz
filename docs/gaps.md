...
1. **Checker and emit residuals** — `Map.empty`, `Set.empty`, and `List.empty` still use a bare constructor. A Queue or Deferred handle has no payload until the first offer or complete. Param letters (`A`/`E`) still unify. Param and Fun types stay strings. Check parses those strings for compare, subst, alias expand, and tuple vs Fun. A path-dep file over 40k keeps def heads with a stub body so Check can resolve a qualified call.
