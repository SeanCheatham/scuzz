...
- **Observation-first `State`** — observation section first is in (signals, a11y, hits, drive records). Effect log (op names, sizes or hashes, never payloads), fiber census, heap deltas, and fault context enter when a universal oracle or claim needs them. Deterministic capture: creation-order ids, no pointers, no hash-map order. Versioned schema stays open.
- **Terminal-point universal oracles** — double-run same-seed determinism (Headless dump and timeline) is in. Heap returns to baseline, acquire/release pairing, finalizer-on-cancel, no parked fibers at quiescence, and live/verify differential stay open.
