...
Then the staged rewrite slices ([`vision.md`](vision.md#self-hosting)): fmt core is in (`examples/fmt`). The kernel typechecker is in (`examples/tyck`). Kernel codegen is in (`examples/codegen`). CLI core is in (`examples/cli`). Driver core is in (`examples/compiler`). Check dispatch is in. Test/fuzz core is in. Hello package compile is in. Fmt-shaped `if` / String / `Str.concat` IR clang-links and prints `fmt-ok`. Int compare, ptr params, nullary enum construct, and nullary `match` IR clang-links and prints `r`. Int list literals, `List.len`, `List.concat`, and `List.reverse` IR clang-links and prints `2`. Payload enums, match binds, `List.head` / `List.tail` / `List.isEmpty`, and string payload tokens clang-link. String and Int match, `::`, `Str.charAt` / `Str.slice`, and Builder clang-link. `Str.toInt`, 2-tuple construct/match, `List.at`, and record construct/match clang-link. File-stem modules and qualified calls clang-link and print `ab`. Record field access, `Str.repeat`, mixed if-ptr phis, ptr-head `::`, and 2-tuple ptr match match Rust LLVM. Scuzz check of lexer, parser, and fmt sources is in. 3-tuple construct/match nests pairs and peels them. Mixed record and enum payloads unbox only Int fields. Enum case tags use a single match so they stay distinct. A borrowed pointer return retains. Scuzz emit of `examples/fmt` clang-links and prints `fmt-ok`. Scuzz emit of `examples/tyck` clang-links and prints `tyck-ok`. Scuzz emit of `examples/codegen` clang-links and prints `ir-ok`. Scuzz emit of `examples/cli` clang-links and prints `cli-ok`. Scuzz emit of `examples/hello` clang-links and prints `Hello, Scuzz!` / `ready.` `Fs.read` payload `flatMap` binds `%value`. `Drive.compileFiles` emits multi-file IR. `Drive.emitDir` lists `src/` and nested path-dep `src/`. Rust-compiled `Drive.emitDir` compiles `examples/fmt`, `examples/tyck`, `examples/codegen`, and `examples/cli` from disk; those IR clang-link and print `fmt-ok` / `tyck-ok` / `ir-ok` / `cli-ok`. Scuzz emit of `Drive.emitDir` clang-links and compiles `examples/fmt`, `examples/hello`, `examples/tyck`, `examples/codegen`, and `examples/cli` from disk; those IR clang-link and print `fmt-ok` / `Hello, Scuzz!` / `tyck-ok` / `ir-ok` / `cli-ok`. Next is Kernel clang-links from Scuzz emit and prints `tco:0` / `tcom:0` / `build:2097152`. Scale clang-links and prints `mapn:3048` / `hit:0:49`. Scuzz-emitted counter clang-links with Skia. A headless two-frame run writes the same structural dump as `examples/counter/goldens/counter.dump`. Scuzz-emitted studio clang-links with Skia. A headless two-frame run writes the same structural dump as `examples/studio/goldens/studio.dump`. Compile itself is in. Scuzz-emitted CLI compiles hello, the compiler library, and itself. The product CLI is the Rust binary. `v0.1.0` already shipped. The next GitHub Release (`v0.2.0`) is the last-Rust cutoff. After that tag, that `scuzz` compiles going forwards. Rust crates leave after that. Each slice is a Scuzz package with roundtrip, idempotence, and differential oracles. The last Rust `scuzz` ships as a tagged GitHub Release and becomes the bootstrap compiler. The Rust crates retire when that tagged binary compiles the Scuzz toolchain, and that toolchain compiles `examples/` and itself.
