...
- **Net listen** — `Net.serveOnce` one GET; `Net.serve` keeps the listen socket (Stage 0 and self-host; `examples/server`). TestRuntime queues injected paths and drains them. Live listen is nonblocking; the fiber parks on `poll` so other IO can run (`test_io.c`). Connection read/write and `httpGet` still block (code **6**). Later: poll those too.
