...
- **Timeline claim kernel** — first cut is in. Each fuzz run records an observation `State` per pump (signals, a11y, `[last_hit]`, drive records). `*.scuzz_verify` predicates judge at the quiesced terminal point. Runtime claim latches are gone. A failing claim reports the violating state index. Remaining: versioned `State` schema, typed terminal boundary (settled vs quiesce-budget-tripped; a tripped budget fails before claims judge), effect log, fiber census, pair-of-timelines metamorphic.
