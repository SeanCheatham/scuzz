...
- **LSP / editor tooling** — `fmt`, `check --message-format=json`, `watch`, and `scuzz lsp` exist. LSP wraps `check` (didOpen/didChange overlay open buffers; didClose returns to disk; `workspace/didChangeWatchedFiles` republishes check). Positions are UTF-16. Language-server methods use that parse. Unknown `IO.*` methods parse as calls so check reports them and code actions can rename them. JSON diagnostics stay the single schema. `check` reports more than one parse or type error per run. Method catalog: [`vision.md`](vision.md#tooling).
