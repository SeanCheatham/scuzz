...
- **Net** — RFC 8305 IPv6 preference delay is out. `Net.httpGet` queries A and AAAA together and races TCP connects when both exist; CNAME chains re-query both (cap 5). IPv4 literals and `http://[::1]/` skip DNS. `Net.serve` listens on `127.0.0.1` and `::1`.
