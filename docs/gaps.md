...
2. **Checker and emit residuals** — Param letters (`A`/`E`) still unify. Stream, List, Map, Set, Resource, Queue, and Deferred still use a bare constructor as some T. The parser stores Fun/Param types as strings; Check parses them. A path-dep file over 40k keeps def heads with a stub body so Check can resolve a qualified call.
