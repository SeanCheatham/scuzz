...
2. **Compile-time performance** — This is the next thesis gap. `scuzz check examples/compiler` is 20 s. A cold `scuzz build examples/tyck` is 47 s. Emitted string literals intern to pinned allocations. Kit signatures parse to `Ty` when the table is built. Generic kit calls compare those `Ty` values. Remaining cost: RC retain/release churn and `sz_list_concat` in string building. `zipCheck` still shows and parses the accumulated type. Coverage uses the live program when compiled files match live.
