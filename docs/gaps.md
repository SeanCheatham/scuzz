...
1. **Compile-time performance** — `scuzz check examples/compiler` takes about 6 s on this host. `scuzz build --full examples/tyck` takes about 16 s. Measure both commands after each compile-time change. Further work must reduce the cost of checking and emitted LLVM text.
