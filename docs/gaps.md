...
Time parse and zones. Regex capture and replace. Base64. UUID. HMAC. `Map` / `Set` keys beyond `Int` or `String`. `scuzz eval`. Generators. Drive `==` wrap on UI. OS threads. HTTPS serve with app cert and key files.
