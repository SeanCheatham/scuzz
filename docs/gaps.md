...
**Decision deferred.** `scuzz fuzz --iterations 16 examples/io` fails on both engines: the second search of seed 42 reaches `drive composePayloads 9` with `fault_seed = 1`, and the driver surfaces `Fs: injected fault`. CI runs `examples/io` at `--iterations 2` and does not reach it. Open question: the driver tolerates Fs faults, or the scenario declares its fault surface.
