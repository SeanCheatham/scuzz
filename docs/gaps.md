...
1. **Compile-time performance** — `scuzz check examples/compiler` takes about 10.4 s on this host. `scuzz build --full examples/tyck` takes about 15.3 s. Measure both commands after each compile-time change. Emitted string literals intern to pinned allocations. Kit signatures parse to `Ty` when the table is built. Generic kit calls compare those `Ty` values. `zipCheck` and `checkKnownRet` keep `Ty`. Env lookup returns `Ty`. A borrowed tail of a live parameter does not retain. A single-arm match on a borrowed value does not retain a borrowed result. Owned concat reuses a unique left value. A shared or pinned left value is copied. Enum searches walk list tails. Parser token chunks use direct list lookup. Enum tag reads are read-only in emitted IR. The lexer classifies identifiers before punctuation pairs. Remaining cost is spread across the compiler. Line-column lookup walks line starts for each emitted location. `emitProg` builds the LLVM text. Coverage uses the live program when compiled files match live.
