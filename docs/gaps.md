...
- **Oracle idioms in `guide.md`** — Authors write `Timeline => Bool` and drive oracles in `*.scuzz_verify`. Metamorphic swap and concrete-fact `.require` are in. English grammar, Given rows, and intent thunks are deferred with mining. They are not current work.
- **Timeline claim kernel** — first cut is in. Each fuzz run records an observation `State` per pump (signals, a11y, `[last_hit]`, drive records). `*.scuzz_verify` predicates judge at the quiesced terminal point. Runtime claim latches are gone. A failing claim reports the violating state index. Remaining: versioned `State` schema, effect log, fiber census, pair-of-timelines metamorphic.
...
- **Terminal-point universal oracles** — double-run same-seed determinism, heap baseline at the terminal point, acquire/release pairing, finalizer-on-cancel, no parked fibers at quiescence, and live/verify differential are in. A `*.scuzz_sim` overlay is a declared live/verify delta.
- **Coverage axes** — breadth is in: `scuzz check` reports unclaimed defs, signals, and controls as info; the campaign reports reached states that vary in `State` fields no claim reads (`[coverage].unclaimed_varied` in `summary.toml`). Strength: the mutation-survivor split (weak claim in a claimed field, missing claim in an unclaimed field; inert mutants filtered by bit-identical replay). Reachability stays `sometimes` plus unified unfired-trigger failure.
