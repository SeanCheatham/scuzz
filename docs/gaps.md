...
- **Hot reload and debugging tools** — Headless + in-process reload + debug for agents. Headless is a peer; `scuzz watch` only rebuilds; `[ui]` `run --watch` stamp-reloads the View tree (Signals stay), writes `build/debug.dump` (signals + a11y + `[taps]` for `tap N` + `[fields]` for the `text`/`type`/`backspace` target), and plays `build/inject.script`. Still missing: new machine code in-process, and `text N` / `type N` by dump index. Do not document `watch` as hot reload.
