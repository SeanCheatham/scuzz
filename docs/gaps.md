...
- **Servers inside simulation** — direction: drivers or a sim setup mechanism serve and bind, and clients hit those bindings. Transport (loopback vs virtual) is uncodified. Scheduler ownership, not address, stays the determinism boundary.
...
- **Timeline claim kernel** — in. Each fuzz run records an observation `State` per pump (signals, a11y, `[last_hit]`, drive records). Session claims return `Verdict`; an invalid verdict carries the violating state index and evidence. The terminal boundary is typed: a run ends settled or quiesce-budget-tripped, and a tripped budget fails the run before claims judge. The timeline dump carries a version (`# timeline v=1`); the compiler rejects other versions. Runtime claim latches are gone. Remaining: Timeline re-derivation behind the algebra (checkpoint + replay vs retain-all), effect log, fiber census.
- **Observation-first `State`** — observation section first is in (signals, a11y, hits, drive records). Heap deltas feed the terminal heap-baseline and acquire/release oracles. Effect log (op names, sizes or hashes, never payloads), fiber census, and fault context enter when a claim needs them. Deterministic capture: creation-order ids, no pointers, no hash-map order. Versioned schema stays open.
...
- **Studio heap growth under fuzz** — fixed: `Signal.getStr` marked its copy borrowed (leaked per call), and the session baseline ran before unmount, when view-owned capture packs still pin session values. Studio search and heap oracles are green; the widget-catalog row's mutants are claimed (0 survivors at 59/59 sites).
- **Swarm testing + corpus hygiene** — in. Each run overweights one seed-picked event kind (deterministic per seed). `scuzz fuzz --minimize-corpus` greedily drops events while pass/fail status and declared `sometimes` coverage hold; idempotent.
- **Nightly deep campaign** — in: scheduled `nightly-deep-campaign` job (cron + workflow_dispatch) runs `--iterations 512` over counter and studio plus `--relate` on both. No corpus auto-commit; failures report to authors. CI push/PR stays a bounded deterministic campaign.
