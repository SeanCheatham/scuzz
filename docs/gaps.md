...
- **Studio heap growth under fuzz** — `scuzz fuzz` on `examples/studio` fails the heap-baseline oracle (count stable, +6 bytes per session) on corpus replay. Pre-existing; counter and the bad-* examples stay green. Find the leaked allocation.
