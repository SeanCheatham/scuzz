...
**Status.** `scuzz fuzz` proves the mechanism on `examples/` (`bad-example` is the expected-fail campaign). Corpus persistence, value shrinking, and mutant-report UX are the ordered next steps ([`vision.md`](vision.md#open-work)).
...
- **Fault injection and schedule exploration** — TestRuntime fakes can fail the Nth operation and drop stub responses. PCT-style schedule exploration bounds race depth for fiber interleavings. After corpus persistence and value shrinking.
