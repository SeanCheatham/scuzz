...
| Test interpreters | Built-in verification: hermetic TestRuntime fakes (`SCUZZ_TESTRT=1`; no live sockets; `Sys.exec` / `Sys.spawn` fail; `Sys.getenv` sealed; `Sys.alive` / `Sys.kill` fake); `intent.scuzz_intent` + `.require` + stem-paired `*.scuzz_sim` / `*.scuzz_drivers`; `scuzz fuzz --iterations` (search + mutation; shrink + dump determinism; `--oracles` for residual predicates; `corpus/` replay; `--iterations 0` corpus-only) | Wall-clock-only harnesses; ad-hoc FFI mocks; app-level Mockito / `src/test` unit trees; third-party mutation/fuzz frameworks; live network beyond stubs under sim |
