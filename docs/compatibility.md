...
| Test interpreters | Built-in verification: hermetic TestRuntime fakes (`SCUZZ_TESTRT=1`; no live sockets; `Sys.exec` / `Sys.spawn` fail; `Sys.getenv` sealed; `Sys.alive` / `Sys.kill` fake); `*.scuzz_verify` + `.require` + stem-paired `*.scuzz_sim` / `*.scuzz_drivers`; `scuzz fuzz --iterations` (search + mutation; shrink + dump determinism; terminal heap baseline; acquire/release pairing; finalizer-on-cancel; leftover park; live/verify differential; `--oracles` for residual predicates; `corpus/` replay; `--iterations 0` corpus-only) | Wall-clock-only harnesses; ad-hoc FFI mocks; app-level Mockito / `src/test` unit trees; third-party mutation/fuzz frameworks; live network beyond stubs under sim |
...
| Dogfood IDE | `scuzz ide` launches the bundled `[ui]` package. The app consumes `check` / `lsp`. Headless is a peer. Prerequisites: [`gaps.md`](gaps.md) | Self-hosted compiler; second typer; Desktop-only editor; a `scuzz-ide` binary |
