...
| Test interpreters | Built-in verification: TestRuntime fakes (`SCUZZ_TESTRT=1`); stem-paired `*.scuzz_sim` / `*.scuzz_laws`; `scuzz fuzz`; mutation (direction) | Wall-clock-only harnesses; ad-hoc FFI mocks; app-level Mockito / `src/test` unit trees; third-party mutation/fuzz frameworks |
