...
| Packaging | macOS UI `.app` through `scuzz package --target host`; Android debug APK through `scuzz package --target android` (installs when adb lists a device); iOS simulator `.app` through `scuzz package --target ios`; local iOS run and hot reload through `scuzz run --target ios --watch` | Gradle/CocoaPods as Scuzz APIs; Flutter platform channels |
