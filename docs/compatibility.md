...
| Dogfood IDE | `scuzz ide` launches the bundled `[ui]` package. The app consumes `check` / `lsp`. Headless is a peer | A compiler inside the IDE app; second typer; Desktop-only editor; a `scuzz-ide` binary |
...
fragment. An `http://` or `https://` route is a real URL. Images and icons
expose `role=img`. A breadcrumb is a navigation group.
