...
Unknowns and known gaps: [`gaps.md`](gaps.md). Next slices: [`plans.md`](plans.md). Work order: core value types first — `Float` is in. Next is compiler-emitted reference counting, then `Map` / `Set`. Mobile packaging (`scuzz package --target ios` CLI wiring, Android) comes after. Open unknowns: Mobile on Android + real devices (iOS simulator proven), GPU presenters.
