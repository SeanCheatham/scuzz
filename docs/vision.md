...
Current arc. Locks: [`philosophy.md`](philosophy.md#evaluator). Next slice: **Guided tutorial** (7).
...
7. **Guided tutorial.** Next. The evaluator returns a reduction trace with the view: one row per step with the expression span, the binding that changed, and the effect performed, capped, with self tail calls collapsed. Docs renders a trace view, a timeline view of one snippet under two schedule seeds, a coverage overlay on the source, and one mutant verdict, each a `View` that Headless claims assert on. Tutorial pages are manual data with a snippet and a visualization kind per block. Proof: a `bad-*` example typed into the page shows the claim fail under one seed and pass under another, headless and in Chromium.
