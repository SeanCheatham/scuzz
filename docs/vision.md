...
- **Mutation pressures the oracles.** `scuzz fuzz` mutates live `def` and impl method bodies after search. Residual oracles must kill each mutant. `--oracles` mutates residual predicates. Count and apply share one walk, so site numbers match. A typecheck or link miss is killed, not a campaign abort. Each mutant gets an idle probe, then replay of corpus+seeds entries that passed on the original. Kill = idle fails, or a passing entry fails. Survivors that change a claimed `State` field are weak claims. Survivors that change an unclaimed `State` field are missing claims. Survivors with bit-identical replayed timelines are inert and unreported.
