...
runtime size constraint. Studio groups its working pages in an Index Book. The editor uses Live, Verify, and Session as Index Book pages. Live lists file-stem modules. Verify lists claim and scenario files. Session holds check diagnostics and Run/Fuzz output. Outlined secondary actions stay on the app bar.
...
separate working pages. The editor uses Live, Verify, and Session. Native proofs cover page switching, hidden controls,
