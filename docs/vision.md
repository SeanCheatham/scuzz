...
- Payload enums + `record` sugar (methods on generic records and enums) + thin traits/`impl` (static dispatch, including generic records) + monomorphized generics on defs/enums/records
...
Unknowns and known gaps: [`gaps.md`](gaps.md). Next slices: generic `impl` for generic enums — [`plans.md`](plans.md). Open unknowns: device Mobile (blocked on NDK/Xcode), GPU presenters.
