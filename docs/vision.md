...
- **JSON diagnostics** (`scuzz check --message-format=json`) are the editor protocol. `scuzz lsp` wraps that, overlays open buffers, and serves language-server methods from the same parse. Unclaimed def, signal, and control reports are info. They do not fail `check`. Do not grow a second typer or schema.
...
- **Native make:** quiet on success. A failed `make` prints the log. Clang compiles `.ll` without C `-I`. Package link uses `-O0 -Wno-override-module` (IR has no host triple). `scripts/bootstrap.sh` clang-links the product CLI at `-O2` after tagged bootstrap emit. A cli-ok proof of `examples/cli` writes a separate out-dir so it does not replace that binary. Make and clang of runtime and UI crates use `SCUZZ_HOME` when that env is set. `scuzz run` uses the fingerprint. `scuzz build PATH` writes `PATH/build`. A gpu Skia link adds EGL and GLESv2 on Linux. On Darwin it adds OpenGL. A Darwin UI link uses `-lc++`, Cocoa, `-force_load`, and the Skia frameworks. A fuzz probe that runs longer than 20 s is killed.
...
3. Kernel codegen in Scuzz: LLVM IR text. In: `examples/codegen` lowers the kernel subset (hello, `if`, lists, enums, match). Oracles: emit idempotence (`ir-ok`). `scuzz fuzz --iterations 8 examples/codegen` stays green. Emit lives in `examples/compiler` (path dep).
...
One failure channel: `SzError` on `IO`. Blessed kits only. No app-level `IO.delay`. Cooperative single-threaded fibers (no OS threads for IO). Park on sleep, empty take, incomplete get, and fd poll. Cancel runs `IO.ensure` / `Resource` finalizers. `Fiber.fork` starts a supervised child. TestRuntime (`SCUZZ_TESTRT=1`) fakes clock, random, FS, net, and console. Simulation is hermetic: no live sockets; HTTP uses stubs and a loopback mailbox; `Sys.exec` / `Sys.spawn` fail; `Sys.getenv` is sealed; `Sys.alive` / `Sys.kill` use a fake process table. Live `Net.serve` binds localhost. Live HTTP client kits take `http://` and `https://` with OpenSSL. Expand `Net` on this HTTP/1.0 stack. Do not expose POSIX sockets. Do not add a second HTTP client. TLS is for `https://` on this kit. Every new `Net` op keeps a TestRuntime fake. Surface catalogs: [`guide.md`](guide.md). Panics abort through `sz_panic`.
...
Headless is a **peer** of Desktop/Mobile — product runtime for agents and CI. Frame boundary is `pump`. World effects stay blessed `IO`. No UI feature without a Headless path. Prefer `Signal.list` + `View.each`. Derived display through `Signal.map` + `View.bindText`. Nested declarative construction only. `Ui.run(_ => view)` is the session. Create Signals outside the factory so they stay. Desktop/Mobile records live input to `build/record.script`. Headless replays through `--script`. Dump and script verbs: [`guide.md`](guide.md).
...
Unknowns and known gaps: [`gaps.md`](gaps.md). Hardware device runs stay open. Impeller / Skia GPU raster stay deferred.

`scuzz ide` launches the bundled `[ui]` package (`examples/editor` / `SCUZZ_HOME/ide`). Headless is part of every UI slice. Remaining IDE gaps: [`gaps.md`](gaps.md). `scuzz test --differential` compares structural dumps across render backends.
