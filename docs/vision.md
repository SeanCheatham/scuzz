...
Unknowns and known gaps, ranked by risk: [`gaps.md`](gaps.md). Closed in-tree slices: stem-paired laws/sim, desktop Window typing (`SZ_INPUT_TEXT_EDIT`), Stage-0 spans, alloc accounting / optional `test-asan`, Stage-0 unary payload ADTs, self-host unary payload ADTs for user programs (`examples/adt`), cooperative fiber `race`/`both`/`Queue`/`Deferred` with TestRuntime virtual-time jumps, long-lived Headless pump flatness (counter-shaped `Signal.map` path). Still open: self-host span port, rewriting compiler IR/AST onto payloads, file-as-module namespaces, device Mobile, non-path deps. **Blocked on external assets/toolchains:** real Skia text (needs hosted `SCUZZ_SKIA_URL` prebuilt), GPU presenters (downstream of Skia). Deeper UI (Flutter-style constraints) and Windows desktop stay deferred. IME waits on real text rendering.
