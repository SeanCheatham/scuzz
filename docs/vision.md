...
| `scuzz fuzz` | `--iterations N` campaign. Replay corpus then seeds. `--iterations 0` is corpus-only. Each run quiesces, then claims judge the terminal timeline in memory. Search failure fails the campaign. `--iterations > 0` also fails when a declared `Property.sometimes` name or `afterHit` / `onHit` trigger never fires. `--iterations 0` writes those names in `summary.json` and does not fail. `--no-fail-fast` keeps that repro, finishes search, then mutates. `--oracles` mutates residual predicates. Survivors remain advisory unless `[fuzz].score_floor` is set. Inert mutants are unreported. Writes `build/fuzz/summary.json` (schema `v=1`, kind `fuzz`) with sometimes and triggers sections (`declared` / `reached` / `never`). Flags and campaign steps: run `scuzz docs verify` |
