...
- **Mutation pressures the oracles.** `scuzz fuzz` mutates live `def` and impl method bodies after search. Residual oracles must detect behavioral changes. The summary separates killed, survived, inert, and invalid mutants. The score is killed / (ran - invalid - inert). It is absent when that denominator is zero. `--oracles` mutates residual predicates. Count and apply share one walk, so site numbers match. A typecheck or link failure is an invalid mutant. It does not abort the campaign. Each mutant gets an idle probe, then replay of corpus+seeds entries that passed on the original. Kill = idle fails, or a passing entry fails. A surviving mutant prints its kind and source location (`Module.def:offset`). Survivors stay advisory unless the package sets `[fuzz].score_floor`. The campaign fails when the score drops below that floor. `examples/counter` sets `0.500`. Do not set a floor on the editor package. `--iterations 0` does not mutate. Mutation covers every site when `sites <= 64`. A larger set rotates by the campaign seed. Survivors with bit-identical replayed timelines are inert and unreported.
...
| `scuzz fuzz` | `--iterations N` campaign. Replay corpus then seeds. `--iterations 0` is corpus-only. Each run quiesces, then claims judge the terminal timeline in memory. Search failure fails the campaign. `--iterations > 0` also fails when a declared `Property.sometimes` name or `afterHit` / `onHit` trigger never fires. `--iterations 0` writes those names in `summary.json` and does not fail. `--no-fail-fast` keeps that repro, finishes search, then mutates. `--oracles` mutates residual predicates. Survivors remain advisory unless `[fuzz].score_floor` is set. Inert mutants are unreported. Writes `build/fuzz/summary.json` (schema `v=1`, kind `fuzz`) with sometimes and triggers sections (`declared` / `reached` / `never`) and a breadth section (varied buckets, claimed Timeline readers, unclaimed). Breadth prints as info. It does not fail the campaign. Flags and campaign steps: run `scuzz docs verify` |
