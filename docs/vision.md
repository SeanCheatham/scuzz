...
Unknowns and known gaps, ranked by risk: [`gaps.md`](gaps.md). Closed in-tree slices: stem-paired laws/sim, desktop Window typing (`SZ_INPUT_TEXT_EDIT`), Stage-0 spans, alloc accounting / optional `test-asan`, Stage-0 unary payload ADTs. Still open: real Skia text, concurrent IO determinism, long-lived RSS proof, self-host payload + span ports, file-as-module namespaces, device Mobile, GPU presenters, non-path deps. Deeper UI (Flutter-style constraints) and Windows desktop stay deferred. IME waits on real text rendering.
