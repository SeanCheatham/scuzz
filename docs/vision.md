...
Ranked list: [`gaps.md`](gaps.md). Next: branch coverage. The fuzz collect and coverage phases still re-parse package sources in one campaign. Profile that path before more campaign-cost work. Keep type preservation through kits and emitted code as the first gate. Extend native composition proofs before more kit surface. Accepted compositions must compile and preserve values. Invalid compositions must fail `check`.
