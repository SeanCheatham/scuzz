...
- **AI-Friendly**: Headless, hot reload, and debugging tools aid agents. Headless is a peer runtime. `scuzz watch` only rebuilds. `[ui] run --watch` is hot reload: it stamp-reloads Views, writes `build/debug.json`, and plays `build/inject.json`. Desktop/Mobile `scuzz run` exports `SCUZZ_UI_RECORD` and `SCUZZ_UI_DEBUG_DUMP`. The session records live OS input to `build/record.json` and writes `build/debug.json`. Replay Headless with `scuzz run --headless --script build/record.json --dump build/debug.json`. Stamp-watch keeps Signals. IO-only `run --watch` kills and reruns on source change. `scuzz lsp` wraps `scuzz check` JSON diagnostics. Panic, goto-def, and rename must use Scuzz source spans. Dump, inject, fuzz verdict, and coverage are one typed session schema. A dump path writes schema `v=2` (`kind` `dump`) at any suffix. The dump covers signals, `views` a11y preorder lines, an `a11y` forest, taps, fields, editors, splits, overlays, scrolls, hits, session, heap, and live. A11y nodes carry `role` and `label`, toggle kinds carry `on`, numeric kinds carry `value`, and children nest under `children`. Signal `value` and `list` payloads encode typed (strings, numbers, arrays, ADTs as `tag` plus `payload`). Timeline states record the same signals array as their observation. A script, record, or inject path uses schema `v=1` (`kind` `inject`). The path must end in `.json`. The inject document is one object per event with a pump-after-event rule. A record rewrites the whole document on each live event. A watch inject plays the whole document on change. `scuzz fuzz` writes `build/fuzz/summary.json` (`kind` `fuzz`) with fuzz, corpus, classify, mutate, coverage, sometimes, triggers, and breadth sections. Dump and inject ops: run `scuzz docs commands`.
...
- **Dogfood IDE.** A Scuzz `[ui]` package is the in-tree IDE. `scuzz ide` on the one CLI launches that package with Desktop. Headless stays a peer (`scuzz ide --headless`). The chrome is Live, Verify, and Session landmarks. Choice chips stay unnumbered. The editor does not use Index Book chapter numbers, Start tiles, or Back/Next sibling chapters. Live lists `*.scuzz` stems. Verify lists claim names; the scenario file may stay listed. Session holds named campaign chips plus check diagnostics and Run. Fuzz spawns `scuzz fuzz --iterations 0` and polls dump files. Named chips are the fuzzer map (declared versus reached). Check stays on Live. Live has no document tab row. The app-bar title is the package name. The app bar keeps Save and Check. The app talks to `scuzz check` / `scuzz lsp` / `scuzz fmt` / `scuzz run` / `scuzz fuzz`. It does not reimplement them. `scuzz lsp` stays the protocol for external editors. Do not add Desktop-only editor behavior. Do not ship a second `scuzz-ide` binary.
...
- **Docs.** The README introduces Scuzz with plain language, a complete example, and one install-to-run path. It states current platform limits and links to the manual and checkout setup. `scuzz docs` prints the technical manual from `examples/manual`. Kit rows come from `examples/compiler/src/Kits.scuzz`. LSP hover uses the same kit formatter. The hosted Docs GUI (`examples/docs`, scuzz.build) and CLI `scuzz docs` keep the same topic ids and coverage. They are not one document with two skins. Do not require the GUI to reuse CLI copy, chrome, or UX. Start tiles, Index copy, and page layout may differ. There is no `guide.md` or generated `kits.md`.
...
- **Native make:** quiet on success. A failed `make` prints the log. Clang compiles `.ll` without C `-I`. Package link uses `-O0 -Wno-override-module` (IR has no host triple). `scripts/bootstrap.sh` clang-links the product CLI at `-O2` after tagged bootstrap emit. Runtime and UI crate makes use `-j` for host cores. `scuzz run` and the fuzz link path run those makes together with the Scuzz emit. `scripts/bootstrap.sh` makes the runtime while the tagged compiler builds the CLI. A cli-ok proof of `examples/cli` writes a separate out-dir so it does not replace that binary. Make and clang of runtime and UI crates use `SCUZZ_HOME` when that env is set. `scuzz run` uses the fingerprint. `scuzz build PATH` writes `PATH/build`. A fuzz probe that runs longer than 20 s is killed. The CLI uses `IO.timeout`. It does not call the host `timeout` command.
...
- **`Property.sometimes` keeps composition honest.** Reachability accumulates across a fuzz *campaign*. The campaign extracts declared names from live sources at verify compile. It unions reached names across corpus, seeds, and search. Declared-but-never-reached names fail the campaign when `--iterations > 0`. `--iterations 0` reports them and does not fail. `Verdict.afterHit` and `Verdict.onHit` use the same report and fail rule for their hit triggers. A missing hit still leaves that per-timeline fold valid. Oracle-free drivers cannot pass vacuously. It is a coverage/fitness signal for corpus guidance, alongside Headless dump novelty. It is a path marker (`Unit`), not a value method.
- **Mutation pressures the oracles.** `scuzz fuzz` mutates live `def` and impl method bodies after search. Residual oracles must detect behavioral changes. The summary separates killed, survived, inert, and invalid mutants. The score is killed / (ran - invalid - inert). It is absent when that denominator is zero. `--oracles` mutates residual predicates. Count and apply share one walk, so site numbers match. A typecheck or link failure is an invalid mutant. It does not abort the campaign. Each mutant gets an idle probe, then replay of corpus+seeds entries that passed on the original. Kill = idle fails, or a passing entry fails. A surviving mutant prints its kind and source location (`Module.def:offset`). Survivors stay advisory unless the package sets `[fuzz].score_floor`. The campaign fails when the score drops below that floor. `summary.json` then records `fuzz.ok` as false. `examples/counter` sets `0.500`. Do not set a floor on the editor package. `--iterations 0` does not mutate. Mutation covers every site when `sites <= 64`. A larger set rotates by the campaign seed. Survivors with bit-identical replayed timelines are inert and unreported.
- **Expected-fail proof** is the `examples/bad-*` packages. Search failures pin a known-wrong campaign in `corpus/` and write `repro.toml`. `examples/bad-sometimes` fails the campaign when a declared `sometimes` name or hit trigger never fires. It does not write `repro.toml`. `--iterations 0` reports that gap and stays green. `examples/bad-intent` fails `scuzz check`. The others keep `scuzz check` green. `--no-fail-fast` still mutates. Inert mutants stay unreported. Catalog: run `scuzz docs verify`.
...
| `scuzz fuzz` | `--iterations N` campaign. Replay corpus then seeds. `--iterations 0` is corpus-only. Each run quiesces, then claims judge the terminal timeline in memory. Search failure fails the campaign. `--iterations > 0` also fails when a declared `Property.sometimes` name or `afterHit` / `onHit` trigger never fires. `--iterations 0` writes those names in `summary.json` and does not fail. `--no-fail-fast` keeps that repro, finishes search, then mutates. `--oracles` mutates residual predicates. Survivors remain advisory unless `[fuzz].score_floor` is set. A score below that floor sets `fuzz.ok` to false. Inert mutants are unreported. Writes `build/fuzz/summary.json` (schema `v=1`, kind `fuzz`) with sometimes and triggers sections (`declared` / `reached` / `never`) and a breadth section (varied buckets, claimed Timeline readers, unclaimed). Breadth prints as info. It does not fail the campaign. Flags and campaign steps: run `scuzz docs verify` |
...
**Coverage directs attention on three axes.** Reachability: `sometimes` names and claim triggers must fire in the campaign. Breadth: `scuzz check` reports defs, signals, and controls with no claim, and the campaign reports State buckets that vary and the `Timeline.signal*` / `a11yHas` / `lastHitHas` / `driveHas` / `effectHas` names that verify files call. Varied buckets with no reader print as info. They do not fail the campaign. Strength: a surviving mutant that changes observable behavior prints its kind and source location. Survivors stay advisory unless the package sets a score floor. Survivors with bit-identical replayed timelines are inert and unreported. Gaps report to the author. The campaign reports original source locations and reached flags for live function bodies and branch arms (`if` then/else arms and `match` arm bodies) in `summary.json`. A branch hit is recorded at arm entry with a key of the enclosing def location plus the arm body offset. It maps verification locations back to source. Simulation replacements and mutant probes do not count as live body hits. `sometimes` stays a path marker. Source coverage is not a substitute for named claims.
...
separate working pages. The editor composes unnumbered Live, Verify, and Session choice chips. It does not use View.indexBook numbered tabs. Check stays on Live. Native proofs cover page switching, hidden controls,
...
once before the UI session. The topic list is created once. Title and page
links read that list. They do not rebuild the manual on each page change.
...
Ranked list: [`gaps.md`](gaps.md). Next: compile-time performance. A profile of the campaign path is in [`gaps.md`](gaps.md): coverage and branch rows look up defs by name in `Check.Ftab`. Emit, coverage, and LSP locations use a per-file line-start index. Mutation probes parse live files once and reuse that tree. Collect rewrite parses each live file once and reuses that tree. Coverage residual rewrite keeps program trees. Coverage reuses the live program when compiled sources match. Emit interns strings by prepend and reverses once so `@.strN` keeps first-seen order. The lexer prepends `Eof` and reverses once so the token list keeps source order. LSP semantic tokens prepend each row and reverse once so `data` keeps first-to-last order. Emit intern and split of interpolated literals use Builder. Bind rewrite of interpolated text uses Builder. Mutation site pick prepends each site and reverses once so pick order stays first-to-last. Campaign event string parse uses Builder. Keep type preservation through kits and emitted code as the first gate. IO combinators, Fiber, Ref, List filter/take, Stream emit/map/compileToList, flatten/flatMap, fold/scan, Map/Set set-get, List/Stream zip, Stream unfold/iterate, List fill/tabulate/append/grouped, Map keys/values/mapValues, List init/distinct/toSet/groupBy, Stream interleave/takeRight/evalTap, List inits/tails/sort/distinctBy, List toMap/diff/setAt, List span/partition/transpose, List slice/patch/indexOfSlice, Set union/intersect/diff, Stream changes/exists, List find predicates, Map/Set exists predicates, `List.sum` / `List.product`, Json query lists, editor mark lists, and `Property.force` instantiate payload types; a mismatch fails `check`. Queue and Deferred still return a bare constructor. Map.empty and Set.empty still return a bare constructor. List.empty still returns a bare `List`. Extend native composition proofs before more kit surface. Accepted compositions must compile and preserve values. Invalid compositions must fail `check`.
