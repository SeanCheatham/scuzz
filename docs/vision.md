...
Close thesis-critical gaps in this order: `Signal[T]`, Scuzz spans on panic and LSP, typed `E`, typed agent session schema. Ranked list: [`gaps.md`](gaps.md). `check` diagnostics already use recorded spans. Panic, goto-def, rename, hover, and completion still miss the token.
...
| LSP span misses the token | One JSON schema. Check diagnostics use recorded file:line. Goto-def, rename, and panic still need that span. The dogfood IDE consumes that schema |
