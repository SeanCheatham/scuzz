...
- Payload enums + `record` sugar (methods on generic records and enums) + thin traits/`impl` (static dispatch, including generic records and enums) + monomorphized generics on defs/enums/records
...
Unknowns and known gaps: [`gaps.md`](gaps.md). Next slices: richer trait methods on generic types — [`plans.md`](plans.md). Open unknowns: device Mobile (blocked on NDK/Xcode), GPU presenters.
