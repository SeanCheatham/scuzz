...
- **AI-Friendly**: Headless, hot reload, and debugging tools aid agents. Headless is a peer runtime. `scuzz watch` only rebuilds. `[ui] run --watch` is hot reload: it stamp-reloads Views, writes `build/debug.dump`, and plays `build/inject.script`. Desktop/Mobile `scuzz run` exports `SCUZZ_UI_RECORD` and `SCUZZ_UI_DEBUG_DUMP`. The session records live OS input to `build/record.script` and writes `build/debug.dump`. Replay Headless with `scuzz run --headless --script build/record.script --dump build/debug.dump`. Stamp-watch keeps Signals. IO-only `run --watch` kills and reruns on source change. `scuzz lsp` wraps `scuzz check` JSON diagnostics. Panic, goto-def, and rename must use Scuzz source spans. Dump, inject, fuzz verdict, and coverage become one typed session schema. The dump and inject surfaces landed: a script, record, or inject path that ends in `.json` uses schema `v=1` (`kind` `inject`). A dump path that ends in `.json` uses schema `v=2` (`kind` `dump`). The dump covers the same sections as the text dump plus an `a11y` forest: nodes carry `role` and `label`, toggle kinds carry `on`, numeric kinds carry `value`, and children nest under `children`. Signal `value` and non-string `list` payloads encode typed (strings, numbers, arrays, ADTs as `tag` plus `payload`). The inject document is one object per text verb with the same pump-after-event rule. A `.json` record rewrites the whole document on each live event. A `.json` watch inject plays whole on change. The fuzz verdict surface landed: `scuzz fuzz` writes `build/fuzz/summary.json` (`kind` `fuzz`) with fuzz, corpus, classify, mutate, and coverage sections next to `summary.toml`. The schema now covers every surface. Text dump, script, and summary stay until the text formats retire. Dump and script verbs: run `scuzz docs commands`.
...
| `scuzz fuzz` | `--iterations N` campaign. Replay corpus then seeds. `--iterations 0` is corpus-only. Each run quiesces, then claims judge the terminal timeline in memory. Search failure fails the campaign. `--no-fail-fast` keeps that repro, finishes search, then mutates. `--oracles` mutates residual predicates. Survivors remain advisory unless `[fuzz].score_floor` is set. Inert mutants are unreported. Writes `build/fuzz/summary.toml` and `build/fuzz/summary.json` (schema `v=1`, kind `fuzz`). Flags and campaign steps: run `scuzz docs verify` |
