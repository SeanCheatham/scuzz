...
- Blessed kits + `Signal` / `View` / `Ui` / `Law.*` / `.require` as documented in the guide. Kit lambdas bind the list element type (`List.filter` / `List.filterNot` / `List.map` / `List.flatMap` / `List.find` / `List.exists` / `List.count` / `List.takeWhile` / `List.dropWhile` / `List.forall`), `String` (`View.each` / `Stream.*` / `Resource` / `Net.serve`), or `Int` (`Signal.map`). The lambda body must return the kit result (`View`, `Bool`, `String`, `T` for `List.map`, `List` for `List.flatMap`, or `IO`)
