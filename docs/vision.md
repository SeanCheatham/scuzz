...
Unknowns and known gaps: [`gaps.md`](gaps.md). Next slices: verification pivot (drivers → refinements), then `IO.timeout` (then language `Fiber`, then `forever` / `repeatN` / `retryN`) — [`plans.md`](plans.md). Open unknowns: device Mobile (blocked on NDK/Xcode), GPU presenters.
