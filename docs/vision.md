...
Direction for data/interfaces (see also [`compatibility.md`](compatibility.md)): payload **enums** / case-like records + thin **traits**-as-interfaces; monomorphize generics early. Stage 0 and self-host (`compiler-scuzz/`) emit N-field `Int`/`String` payload enums for user programs (multi-field as `List` in `sz_adt_payload`); compiler sources still use nullary / List+string-tag IR. No classes (mutable identity), no `var`, no classpath/`com.foo.bar` directories. Path deps remain the unit of reuse. Stage 0 namespaces defs by file stem (`examples/modules`); enums stay global. Self-host still merges bare def names (modules example is Stage 0–only until ported). No `private`/`pub` / `import` yet.
