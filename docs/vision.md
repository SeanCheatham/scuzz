...
**Flutter-style constraints** (constraints down, sizes up). Tight slots: `sized`, `aspectRatio`, percent axes on `fraction`, and `expanded` flex. Column/row do not stretch non-flex children. Do not drift into CSS-ish ad-hoc rules. Do not grow Flutter-style constraint-overflow dumps. Diagnose through structural dumps + laws. Shipped widgets (`column` / `row` / `stack` / `expanded` / `center` / `align` / `positioned` / `padding` / `sized` / `minSize` / `background` / `aspectRatio` / `fraction`): [`guide.md`](guide.md).
