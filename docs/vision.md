...
| Topic | Choice |
| --- | --- |
| UI testing / CI | In-source properties + composed drivers through `scuzz fuzz --iterations` (primary; mutation is a phase); structural goldens as regression face; PNG optional through `--pixels` |
...
| Tests | One built-in strategy: **mutation + fuzz + properties + sim + determinism** (TestRuntime). Oracles live **in source** (properties, inline checks, refinements). **Drivers** are oracle-free workloads the fuzzer composes. No classical unit-test culture. No external test frameworks |
...
- **Mutation pressures the oracles.** `scuzz fuzz` mutates live `def` bodies after search. Residual oracles must kill each mutant. `--oracles` mutates residual predicates. A surviving mutant reports the source location, enclosing def, mutation label, and the nearest residual oracle. Survivors mean weak, unreached, or missing properties/refinements.
- **Expected-fail proof** is `examples/bad-example`. `bump` is wrong. Search drives `bumpIncreases`, shrinks the arg, and writes `repro.toml`. A shrunk entry lives in `corpus/` (`drive bumpIncreases 0`). Later campaigns replay it before search. `scuzz check` and `scuzz test` still pass. `--no-fail-fast` still mutates. A failing search keeps no passing corpus, so idle mutants survive when residual oracles do not fire on `@main`. `scale` has no oracle. A surviving `scale` mutant names `scale` and reports no observing oracle. After you fix `bump`, search passes and mutants of `scale` still survive.

| Phase | Role |
| --- | --- |
| `scuzz check` | Format-verify `src/`; typecheck live + properties/refinements + sim + drivers; properties pure, drivers `IO`, no oracles in drivers; every nullary `property` must appear in a `.require`; parameterized properties stay generator-friendly; sim bindings match live types/purity. Reports every parse and type diagnostic in the run |
| `scuzz build` (verify graph) | Layer `*.scuzz_sim` over live defs; compile drivers; residualize `.require` / `where` checks (armed under TestRuntime / fuzz / mutation only); publish parameterized properties as drive targets |
| `scuzz fuzz` | `--iterations N` campaign. Replay `<pkg>/corpus/*.toml` first (sorted names). `--iterations 0` is corpus-only: no search, no mutation. Search: probe, then exhaustive `[ui]` deepening while the next full depth fits, then coverage-guided random. Passing corpus entries seed in-memory keeps. Shrink a failing prefix, then shrink `drive` args, before `repro.toml` and promote it into `corpus/` (hash of `schedule_seed` + events). Persist a keep that reaches a `sometimes` name no stored entry reaches (cap: declared-name count). Dump-novelty keeps stay in memory. Default stops at the first search failure. `--no-fail-fast` keeps that repro, finishes search, then mutates. `[ui]` kept prefixes run twice; dump mismatch fails. Then mutation: idle probe plus corpus replay. `--oracles` mutates residual `Property.check` / `Property.assert` / `.require` predicates. Per-run oracles: `.require` residuals, refinements, panic/`SzError`. Per-campaign: `Property.sometimes` reachability. Summary splits names not reached in this budget from names no stored entry reaches. Survivors fail. A surviving mutant reports location, def, mutation, and nearest oracle. Writes `build/fuzz/summary.toml` with a `[corpus]` table and `[mutate].survivors` |
| Later (optional) | Discharge trivial property/refinement fragments statically; leave the rest as search |
...
```text
src/
  Todo.scuzz              # live module: defs + properties + refinements (all oracles)
...
- Live/`scuzz run` loads `*.scuzz` only. Properties, `.require`, inline checks, and refinements erase.
- No separate properties file. Oracles belong next to the code they constrain, in `*.scuzz`.
...
Deterministic TestRuntime + (for `[ui]`) Headless event scripts (plus sim overlays when present). The fuzz alphabet is `[taps]` targets, text fields, and scrolls **plus declared drivers** (`drive <name> [args]` extends the script line protocol; the verify build publishes the driver table alongside the a11y dump) **plus parameterized properties** (same `drive` verb). A simple Int `where` bound joins the `i` token (`noteDrive i>=0`) and clamps draws. Oracles: in-source **properties/refinements** first. Panic/`SzError` still fails. `Property.sometimes` reachability judges the campaign. Structural dumps aid diagnosis (PNG last). `repro.toml` records a **shrunk** event list + driver invocations, so replay is generator-independent. Shrink removes events first, then shrinks args inside surviving `drive` lines (Int toward the published bound or 0, Bool to `false`, String to `a`). Every campaign loads `<pkg>/corpus/*.toml` (sibling of `goldens/`; same repro shape) in sorted-name order and replays those entries before search. A failing stored entry is a search failure; the stored file is the repro. Passing entries seed in-memory `UiKeep` / `IoKeep`. Search then promotes each shrunk failure into `corpus/` under a content-hash filename (`schedule_seed` + events; idempotent). A keep that reaches a `sometimes` name no stored entry reaches is also persisted (bounded by the declared-name count). Dump-novelty keeps stay in memory. `--iterations 0` replays the corpus and stops. An empty or missing `corpus/` is a no-op. `[ui]` kept prefixes run twice; a dump mismatch fails the campaign. `tap N` / `scroll N` follow a11y preorder (`[taps]` / `[scrolls]`), not a pixel scan. `tap N` activates that target directly so a control below the viewport still fires. Headless fuzz stays stable across fonts. `pump` is time. No hidden nondeterminism. Determinism makes any failing prefix replayable. Seeded `--iterations N` splits the budget: about two thirds search, the rest mutation slots. Search tries exhaustive `[ui]` event scripts depth by depth while a full depth fits, then keeps `[ui]` prefixes (with their schedule seed) that hit new `Property.sometimes` names or a new Headless `dump.txt`, and IO-only schedule seeds that hit new sometimes names, then extends/perturbs them (CLI-only; no runtime machinery). Search always includes `pump`. Default stops at the first search failure. `--no-fail-fast` keeps that repro, finishes search, then mutates. Mutation compiles one mutant per slot, idle-probes, then replays the corpus. A surviving mutant prints file:line, enclosing def, mutation label with a source excerpt, and the nearest residual oracle (same def, else closest span in the same module, else no observing oracle). `[mutate].survivors` in `summary.toml` records site, def, location, label, and oracle. `--replay` restores a shrunk event list. `--oracles` mutates residual predicates. The command writes `build/fuzz/summary.toml` (coverage, mutation, `[mutate].survivors`, and `[corpus]`: `entries` / `failures` / `reached` / `promoted`). Missing `sometimes` names split into not-reached-in-this-budget vs reached-by-no-stored-entry. Flags, script verbs, and schedule seeds: [`guide.md`](guide.md).

Corpus files stay generator-independent event lists. The observation surface is files and env (`sometimes.reached`, scripts, dumps). An external conductor can drive the same binary. `Property.sometimes` verdicts match Antithesis campaign aggregation: a name must occur at least once in the campaign. Scuzz does not emit an Antithesis SDK JSONL stream.

### Layout model

**Flutter-style constraints** (constraints down, sizes up). Tight slots: `sized`, `aspectRatio`, percent axes on `fraction`, `expanded` flex, and opt-in `stretch` (cross axis). Scroll content is unbounded on the pan axis (`max` 0). Column/row do not stretch non-flex children unless wrapped in `View.stretch`. Device-pixel paint multiplies author px by the backing scale so taps match the pixels. Desktop and Mobile present that pixel buffer into a point-sized window. Taps stay in logical points. Nested constructors only. Do not drift into CSS-ish ad-hoc rules. Do not grow Flutter-style constraint-overflow dumps. Diagnose through structural dumps + properties. Widget catalog: [`guide.md`](guide.md).

### UI testing

**In-source properties + composed drivers through `scuzz fuzz`** are primary for `[ui]` apps. Mutation is a phase of that command. **Structural goldens** (Headless signal + a11y dumps) are a regression face — few, live graph, not a substitute for properties. PNG optional (`scuzz test --pixels`). IO packages: properties + drivers + sim under TestRuntime when present. Otherwise compile + TESTRT exit-0 smoke.
...
IDE prerequisites run as two tracks. Track A is input and editor (key alphabet, caret, hover, selection, `View.editor`, viewport, undo, chrome primitives). Track B is kits (Fs entries and paths, delete/rename/walk, `Sys.exec` capture, Json, watch or poll, child stdio). Headless is part of every UI slice. Kits run in parallel with input and editor. Prove editor-scale on `examples/editor` before `scuzz ide`. Do not start the IDE package until those tracks prove unknown 3. Close-lists: [`gaps.md`](gaps.md). Verification items below (list generators, seed tables, fault injection) stay listed. They do not block the input/editor or kit tracks.
...
1. **Generate lists and records.** Draw generator-friendly collection values.
2. **Seed-table idiom.** Feed known input/output pairs into the fuzz corpus as seeds and arm them as oracles.
3. **Fault injection.** Fail the Nth `Fs.write`. Drop a stub response through TestRuntime fakes.
4. **PCT-style schedule exploration.** Bound race depth for fiber interleavings.
5. **Differential structural dumps.** Compare dumps across render backends.
...
| Properties become brittle dump goldens | Properties talk to named module/signal surface. Strict sim/live pairing in `check`. Mutation kills weak oracles |
...
| Dogfood IDE before editor primitives | Close IDE prerequisites in [`gaps.md`](gaps.md) first (two tracks; Headless in every UI slice; `examples/editor` before the launcher). Do not start `scuzz ide` on a one-line TextField |
