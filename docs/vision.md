...
- **AI-Friendly**: Headless, hot reload, and debugging tools aid agents. Headless is a peer runtime. `scuzz watch` only rebuilds. `[ui] run --watch` is hot reload: it stamp-reloads Views, writes `build/debug.dump`, and plays `build/inject.script`. Desktop/Mobile `scuzz run` exports `SCUZZ_UI_RECORD` and `SCUZZ_UI_DEBUG_DUMP`. The session records live OS input to `build/record.script` and writes `build/debug.dump`. Replay Headless with `scuzz run --headless --script build/record.script --dump build/debug.dump`. Stamp-watch keeps Signals. IO-only `run --watch` kills and reruns on source change. `scuzz lsp` wraps `scuzz check` JSON diagnostics. Panic, goto-def, and rename must use Scuzz source spans. Dump, inject, fuzz verdict, and coverage become one typed session schema. The dump and inject surfaces landed: a dump path or a script, record, or inject path that ends in `.json` uses schema `v=1` (`kind` `dump` or `inject`). The dump covers the same sections as the text dump. The inject document is one object per text verb with the same pump-after-event rule. A `.json` record rewrites the whole document on each live event. A `.json` watch inject plays whole on change. List/value signal payloads and a11y lines stay opaque strings in `v=1`. The fuzz verdict surface landed: `scuzz fuzz` writes `build/fuzz/summary.json` (`kind` `fuzz`) with fuzz, corpus, classify, mutate, and coverage sections next to `summary.toml`. Text dump, script, and summary stay until the schema covers every surface. Dump and script verbs: run `scuzz docs commands`.
