...
- `if (cond) then else else` — a branch that starts with `val` is a full block (leading vals, mid-block statement exprs, final expr); otherwise the branch is a single expression so `val x = if … else e` does not steal following vals
- `match { case Enum.Case => expr; case _ => expr }` (Stage 1 parses/prints for `fmt`; emit remains Stage-0 until match codegen is ported)
...
- Builtins: `Str.*` (incl. `lines`), `List.*` (incl. `append`), `Fs.read`/`write`/`list`/`mkdirs`, `Sys.args`/`exec`/`getenv`,
  ...
  `Signal.*` (int/str/list get/set), `View.*` (incl. `button` / `showWhen` / `addTexts` / `clearChildren` / `setTexts`), `Ui.run`,
