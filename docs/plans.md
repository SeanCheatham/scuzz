...
1. ~~Buildable iOS sim shell~~ **Done.** Live Mobile pump loop (`sz_mobile_alive`), ObjC shell under `shells/ios/` (present, touch, keyboard show/hide), `build_sim.sh` builds runtime + sk_sw + app `.ll` for `arm64-apple-ios-simulator` into a signed `.app`. Proven: counter mounts `UiRuntime.Mobile` in a booted sim and renders live (screenshot).
