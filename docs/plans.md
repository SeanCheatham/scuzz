...
CLI core, driver core, test/fuzz core, and hello package compile are in (`examples/cli`, `examples/compiler`, `cli-ok` / `ir-ok`). Hello-shaped IR clang-links against `libscuzz_rt.a`. Scuzz emit of `examples/codegen` clang-links and prints `ir-ok`. Scuzz emit of `examples/cli` clang-links and prints `cli-ok`. The product CLI is still the Rust binary.
