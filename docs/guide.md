...
- Types: `Unit`, `Int`, `Float`, `String`, `Bool`, `List[T]`, `IO[T]`, `A => B`, `Fiber[A]` / `Ref[A]` / `Queue[A]` / `Deferred[A]` / `Resource[A]` / `Stream[A]`, nominal enums. `true` / `false` are `Bool`. `Bool` is not `Int`. `if` needs `Bool`. Comparisons and `&&` / `||` return `Bool`. `attempt` returns `IO[Result[A]]` (`enum Result[T]: Err(msg: String) | Ok(value: T)`). `handleErrorWith(err => …)` binds `err` as `String`. `Float.fromInt(n)` and `Float.toInt(x)` convert (truncate toward zero). `s"$x"` interpolates `String`, `Int`, and `Float`.
