...
| `examples/todo` | `Signal.list` + `View.each` row mapper, Done (`#` prefix) via `List.map` / `Str.startsWith`, Del via `List.filter`, Rename via `List.setAt`, Column `View.expanded` scroll, Fs load/save + laws + drivers / `Law.sometimes` |
