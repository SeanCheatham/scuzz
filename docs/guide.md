...
- Blessed impurity only: `IO.println` / `sleep` / `fail` / `pure` / `race` / `both`, `Ref.*` / `Queue.*` / `Deferred.*` (String payloads), `Resource.make` / `Resource.use` (String payload), `Stream.emit` / `emits` / `eval` / `concat` / `evalMap` / `filter` / `take` / `drop` / `compileToList` / `drain` (String payload), `Fs.*`, `Sys.args` / `Sys.readLine` / `Sys.exec` / `Sys.spawn` / `Sys.alive` / `Sys.getenv`, `Clock.*`, `Random.*`, `Net.httpGet` / `Net.serveOnce` / `Net.serve`
...
| `examples/stream` | `Stream.emit` / `evalMap` / `filter` / `take` / `drop` / `compileToList` / `drain` |
