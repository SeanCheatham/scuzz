...
- `scuzz mutate --limit N` (Stage 1/2 CLI) mutates residual `Law.check` / `Law.assert` / `.require` predicates (negate, flip `==`/`</<=/>/>=/&&/||`, swap `+`/`-` and `*`/`/`, drop `&&` conjuncts, swap `0`↔`1`). Each mutant gets an idle TestRuntime probe, then `--iters` fuzz scripts (`[ui]`) or schedule seeds (IO). Kill = any probe fails; survivors mean weak or unreached oracles. `--iters 0` is idle only. No sites (e.g. `examples/hello`) exits 0. Do not add external mutators
