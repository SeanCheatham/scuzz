...
- Thin **traits** / `impl` with static dispatch (`p.show()` / `p.getOrElse(0)` — including `impl Get[Int] for Point` and `impl Get for Opt`; see `examples/trait`)
...
| `examples/trait` | `trait` / `impl` + `p.show()` / `p.getOrElse(0)` / `o.getOrElse(0)`, including `impl Get[Int] for Point` |
