...
| `examples/studio` | Desktop stay-open app: `showWhen` pages, `Signal.list` + `View.each`, Done/Add/Del/Rename, `View.checkbox`, Fs load/save, `record` / `trait` / stem modules, laws + drivers / `Law.sometimes`. `scuzz run` opens a window (q or Esc quits). `--headless` snapshots. |
