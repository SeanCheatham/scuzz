...
- Thin **traits** / `impl` with static dispatch (`p.show()` — including `impl Show for Box` / `impl Show for Opt` / `trait Get[T]` with `impl Get for Opt` and `getOrElse(default: T): T`; see `examples/trait`)
...
| `examples/trait` | `trait` / `impl` + `p.show()` / `o.getOrElse(0)` static dispatch, including `trait Get[T]` on generic enums |
