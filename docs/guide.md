...
| `examples/todo` | `Signal.list` + `View.each` row mapper, Done (`#` prefix) via `List.map`, Del via `List.filter`, Column `View.expanded` scroll, Rename with `setAt`, Fs load/save + laws |
