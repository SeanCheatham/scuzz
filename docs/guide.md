...
`scuzz package --target host` writes a Mobile host shell under `build/package/host`. `scuzz package --target android` copies the Android template. `scuzz package --target ios` builds a signed iOS simulator `.app` (needs Xcode). Missing Xcode fails with one install line. The iOS shell sends typed text to TextField.
...
- Literals: ints, floats (`1.5`), `true` / `false`, strings, `()`, `s"…$x…"`, list literals `[a, b]`
- Types: `Unit`, `Int`, `Float`, `String`, `Bool`, `List[T]`, `Map[K, V]`, `Set[T]`, `IO[T]`, `A => B`, `Fiber[A]` / `Ref[A]` / `Queue[A]` / `Deferred[A]` / `Resource[A]` / `Stream[A]`, nominal enums. `true` / `false` are `Bool`. `Bool` is not `Int`. `if` needs `Bool`. Comparisons and `&&` / `||` return `Bool`. `attempt` returns `IO[Result[A]]` (`enum Result[T]: Err(msg: String) | Ok(value: T)`). `handleErrorWith(err => …)` binds `err` as `String`. `Float.fromInt(n)` and `Float.toInt(x)` convert (truncate toward zero). `s"$x"` interpolates `String`, `Int`, and `Float`. `Map.set` / `Map.getOrElse` / `Map.contains` and `Set.add` / `Set.contains` are persistent (keys `Int` or `String`).
...
- Prefer sim overlays for app policy (API base URL, a `Backend` value). Blessed kits stay one implementation with TestRuntime fakes on the wire. Simulation is hermetic: TestRuntime does not open live sockets. `Net.httpGet` uses stubs. `Net.serve` uses injected paths. `Sys.exec` and `Sys.spawn` fail. Live `Net.serve` binds localhost. Live `Net.httpGet` may leave the host.
...
- `scuzz mutate --limit N` mutates live `def` bodies (flip `==`/`</<=/>/>=/&&/||`, swap `+`/`-` and `*`/`/`, replace `%` with `*`, drop `&&` conjuncts, swap `if` arms, swap `0`↔`1`, replace an ADT construct with a same-arity sibling). Residual oracles stay armed. `--oracles` mutates residual `Law.check` / `Law.assert` / `.require` predicates instead. Each mutant gets an idle TestRuntime probe, then `--iters` fuzz scripts (`[ui]`) or schedule seeds (IO). Kill = any probe fails. Survivors mean weak or unreached oracles. `--iters 0` is idle only. No sites (for example `examples/hello`) exits 0. Do not add external mutators.
- Deterministic fakes: `TestRuntime` / `SCUZZ_TESTRT=1` for clock/random/FS/network/console in app binaries. Simulation is hermetic (no live sockets; `Sys.exec` / `Sys.spawn` fail)
...
| `examples/kernel` | Language constructs: enums, `record` + `where`, `trait` / `impl`, generics, generic enum/record, stem modules, `private def`, `import`, `Float` |
