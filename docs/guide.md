...
| `examples/studio` | Desktop stay-open app: `showWhen` pages, `Signal.list` + `View.each`, Done/Add/Del/Rename, `View.checkbox` / `View.radio` / `View.slider` / `View.progress` / `View.switch` / `View.chip` / `View.listTile` / `View.badge` / `View.card` / `View.divider` / `View.expansionTile` / `View.iconButton` / `View.verticalDivider` / `View.circularProgress` / `View.avatar` / `View.scrollH` / `View.grid`, Fs load/save, `record` / `trait` / stem modules, laws + drivers / `Law.sometimes`. `scuzz run` opens a window (q or Esc quits). `--headless` snapshots. |
