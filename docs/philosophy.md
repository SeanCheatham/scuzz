...
- A generic def pins its own type parameters for every check in its body. `A` does not match `Int` or `String` there. Call sites still instantiate parameters. Kit argument checks pin the caller type after substitution. A bare list pins its element type when `List.at` on that list is checked against a concrete type. `List.cons`, `::`, `List.concat`, `List.getOrElse`, `List.setAt`, `List.contains`, `List.indexOf`, `List.lastIndexOf`, `List.append`, and `List.intersperse` pin it from a concrete element or list argument. `List.zipAll` pins each bare list from its concrete fill value. A fill that is not concrete leaves that list open. `List.foldLeft` and `List.scanLeft` pin a bare list element when the lambda uses the second name with a concrete seed. `List.foldRight` and `List.scanRight` pin it when the lambda uses the first name with a concrete seed. `Stream.fold` and `Stream.scan` pin a bare stream element when the lambda uses the second name with a concrete seed. An unused element does not pin. `List.map`, `List.flatMap`, `List.filter`, `List.filterNot`, `List.span`, `List.partition`, `List.exists`, `List.forall`, `List.find`, `List.findLast`, and `List.count` pin a bare list element from a concrete parameter annotation, or from a use of that parameter with a concrete type. `IO.foreach` and `IO.foreachDiscard` pin a bare list element the same way. `List.sortBy`, `List.maxBy`, and `List.minBy` pin a bare list element to `Int` when the lambda returns that parameter. A concrete use of the parameter pins it too. `List.groupBy` and `List.distinctBy` pin a bare list element from a concrete use of the parameter. Returning the parameter does not pin. `List.reduceLeft` and `List.reduceRight` pin a bare list element when a lambda name is used with a concrete type. A use of only those names does not pin. A bare map pins its key and value from concrete `Map.set` or `Map.getOrElse` arguments. `Map.contains`, `Map.remove`, and `Map.get` pin a bare map key from a concrete key. The value stays open until `Map.set` or `Map.getOrElse` gives a concrete value. The key stays. `Map.filter`, `Map.exists`, `Map.forall`, and `Map.mapValues` pin an open map value from a concrete use in the lambda. The key stays. `Map.contains`, `Map.remove`, `Map.get`, `Map.set`, and `Map.getOrElse` pin an open key when the value is already concrete. The value stays. `Map.union`, `Map.intersect`, and `Map.diff` pin a bare map from a concrete other map. A bare set pins its element from a concrete `Set.add`, `Set.contains`, or `Set.remove` argument. `Set.union`, `Set.intersect`, `Set.diff`, `Set.isSubset`, and `Set.isDisjoint` pin a bare set from a concrete other set. Two bare sets do not pin. `Set.map`, `Set.filter`, `Set.exists`, and `Set.forall` pin a bare set element from a concrete parameter annotation, or from a use of that parameter with a concrete type. `Stream.map`, `Stream.flatMap`, `Stream.mapConcat`, `Stream.evalMap`, `Stream.evalTap`, `Stream.filter`, `Stream.filterNot`, `Stream.takeWhile`, `Stream.dropWhile`, `Stream.find`, `Stream.findLast`, `Stream.exists`, and `Stream.forall` pin a bare stream element the same way. `Stream.concat` and `Stream.orElse` pin a bare stream from a concrete other stream. Two bare streams do not pin. A call parameter that is a concrete list, map, or set pins a bare argument. Later bindings in that for-comprehension use the pin. Other unbound kit parameters still match through `Type.eq`. Do not add a pin for another kit call.
