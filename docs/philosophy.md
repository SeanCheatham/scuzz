...
- **Scope.** `scuzz eval` runs a package on the host. `scuzz fuzz` searches, mutates, and measures coverage on the evaluator. Docs runs the evaluator compiled to WebAssembly. The Docs decision owns the walkthrough stage spec. Search calls an `oracle` at `Value`. It does not call `Fuzz.probe`. Headless claims assert on those `View`s before a browser does. Check, Cover, and Mutation construct viz when those stages open. A small evaluator warmup of the Counter snippets stays at boot. `examples/manual` is the source for `scuzz docs`. It is not the source for the walkthrough shell. `scuzz run` and `scuzz package` stay compiled.
...
No vendored Skia tree. Thin `sk_capi` (measure + draw). **Default UI backend** is the pinned Skia CPU prebuilt. `SCUZZ_SKIA=sk_sw` is the explicit opt-out. `SCUZZ_SKIA=gpu` rasterizes on OpenGL. It does not paint on the CPU. Structural dumps stay the same. The channel delta against `sk_sw` stays within 2. Impeller stays deferred. Callers depend only on `sk_capi.h`. Editor glyphs use an embedded DejaVu Sans Mono face. `View.text` stays the proportional embedded sans.
...
`Fs.write` replaces a regular file in one operation. The live runtime writes a temporary file in the same directory. It checks write and close errors. It syncs the file. It renames the file over the destination. It syncs the destination directory. A power loss keeps the previous file or the complete new file. An error before replacement preserves the destination. New files use mode 0600. Replacement keeps the access permission bits. It keeps the owner, the group, and the extended attributes when the process can set them. Replacement updates the timestamps. On the live runtime, a symbolic link destination replaces the regular file that the link names. The link stays. A special file destination fails. Simulation applies the same complete-content replacement. A partial-write fault builds the new bytes, discards them, and keeps the previous file.
...
A `View.button` press, a button that becomes shown, a `View.tabs` change, and a `View.each` insert each paint for four pumps. The new tab panel uses the same appear step as a button that becomes shown. The next pump paints the settled tree. A static tree paints one frame and waits. The clock is `pump`. Do not read wall-clock time. Headless and Desktop paint the same frames. The structural dump records the step. The timeline records the hit on the pump that took the input. The session dump keeps that hit. Do not add a spring, a motion language, blur, or a gradient.

**The tree owns views.** A `View` is reference counted. Its parent releases it. A `List[View]` holds views for `View.each`, which mounts the list at layout and frees the views it replaces. A `Signal[List[View]]` that drops a list before any `View.each` mounts it frees the views in that list, so two writes between layouts do not leak the middle list. A list another holder still reads keeps its views. Do not mount a view pulled out of a list signal by hand.
...
- An `impl` supplies each trait method once. Method parameter types and the return type match after trait type arguments and aliases resolve. Parameter names can differ. The one-argument and two-argument `IO` forms use the same `String` error type.
- A generic def pins its own type parameters for every check in its body. A pinned parameter does not match a concrete type. Call sites instantiate parameters. Kit argument checks pin the caller type after substitution. A bare list, map, set, or stream pins an open type argument from a concrete use in that check. Later bindings in that for-comprehension use the pin. Do not add a pin for another kit call. One check binds an unbound value to one concrete type. A second concrete type for that value is an error. `eqPinned` stays the rule inside a generic def.
...
- Constructor patterns compare direct String, Int, and Bool literals before an arm runs. An unqualified case uses the enum type of the matched value. A bare case must belong to that type. The checker rejects an unknown qualified enum case. A qualified enum case must belong to the type of the matched value, including a value in a nested field. A module-qualified record pattern uses the declaring file stem. A `Json` case needs a `Json` value. A `Json.parse` result needs a `Result.Ok` pattern around the `Json` case. Named fields use positions from that enum case. A failed literal comparison tries the next arm. Constructor, tuple, cons, as, and `[]` patterns nest in constructor fields and tuple components.
- A constructor call builds that enum case when the case exists. A module-qualified record constructor uses the declaring file stem. A kit call with the same prefix stays a kit when the name is not a case.
...
- **Behavioral diff.** `scuzz diff [REV] [PATH]` compares a git revision with the working tree. `REV` defaults to `HEAD`. The CLI is the only caller of `git`. It checks out `REV` in a detached worktree under `build/delta/`. The same compiler builds both sides into `build/delta/a` and `build/delta/b`. Both sides run hermetically under TestRuntime. Workloads are the idle probe, the seeds, and the union of both `corpus/` directories deduped by content. Each workload is `same`, `diverged`, `fixed`, `regressed`, or `unrunnable`. The delta names the first diverging timeline state and the changed `signals`, `a11y`, `effects`, `files`, and other sections. States align by index. The report also compares controls, drivers, oracles, claim verdicts, `Property.sometimes` names, and classify counts. `--iterations N` searches the shared alphabet for a timeline or verdict difference. The shrinker keeps that difference. Witnesses go to `build/delta/witness-<hash>.toml`. `scuzz fuzz --replay` reads them. `scuzz diff` writes only under `build/delta/`. It never writes `corpus/`. There is no expected-delta file. The exit code is 1 only for a regression.
...
- **One `scuzz fuzz`, two engines.** Search, mutation, and coverage run on the evaluator. A package with no `[ui]` uses one `scuzz eval --probe DIR` server per file set. A `[ui]` package uses one headless host. The server checks the package once and forks a child per probe. A mutant is a file set, not a link. Corpus replay, `--replay`, `--relate`, `--asan`, and `--live` run compiled. A search failure found on the evaluator replays compiled before the campaign ends. A difference fails the campaign. The idle probe runs on both engines first. A timeline difference, a deadline, or a crash on the evaluator runs every probe compiled and prints why. `SCUZZ_FUZZ_ENGINE=compiled` forces the compiled path. It is the parity control. Search feedback runs on the evaluator only. Every covered `Int` comparison reports `|a - b|`. The search keeps one drive line that lowers the farthest distance at or below 2^16. The distance file keeps the last drive in the script. It nudges one `Int` argument on that line inside its `where` by a power of two sized to that distance. A nearer comparison does not replace that climb. `examples/reach` is the proof. `examples/api-report` uses the same climb on the page cap. `--iterations N` gives five eighths of N to search, rounded down. Mutation uses the rest, up to the site count. Initial probes and corpus replay stay outside that budget. `--iterations 0` is corpus-only. Mutation results persist per site in `.scuzz/mutate.results`, keyed by compiler SHA-256. A def whose body hash changed re-mutates first. A campaign without `[fuzz].score_floor` uses the floor 1.000. Search and corpus failures fail the campaign. `--no-fail-fast` cannot turn a corpus failure into a pass. Catalog: run `scuzz docs verify`.
