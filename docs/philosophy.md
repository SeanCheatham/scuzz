...
- A generic def pins its own type parameters for every check in its body. A pinned parameter does not match a concrete type. Call sites instantiate parameters. Kit argument checks pin the caller type after substitution. A bare list, map, set, or stream pins an open type argument from a concrete use in that check. Later bindings in that for-comprehension use the pin. Do not add a pin for another kit call. `Type.eq` matches an unbound type parameter to any type. The slice in [`plans.md`](plans.md) binds that parameter to one concrete type in one check.
