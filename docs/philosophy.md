...
- Constructor patterns compare direct String, Int, and Bool literals before an arm runs. An unqualified case uses the enum type of the matched value. The checker rejects an unknown qualified enum case. A qualified enum case must belong to the type of the matched value, including a value in a nested field. A `Json` case needs a `Json` value. A `Json.parse` result needs a `Result.Ok` pattern around the `Json` case. Named fields use positions from that enum case. A failed literal comparison tries the next arm. Constructor, tuple, cons, as, and `[]` patterns nest in constructor fields and tuple components.
