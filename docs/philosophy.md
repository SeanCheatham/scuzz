...
- A generic def pins its own type parameters for every check in its body. `A` does not match `Int` or `String` there. Call sites still instantiate parameters. Kit argument checks pin the caller type after substitution. A bare list pins its element type when `List.at` on that list is checked against a concrete type. `List.cons`, `::`, `List.concat`, `List.getOrElse`, `List.setAt`, `List.contains`, `List.indexOf`, and `List.lastIndexOf` pin it from a concrete element or list argument. A bare map pins its key and value from concrete `Map.set` or `Map.getOrElse` arguments. A bare set pins its element from a concrete `Set.add` argument. A call parameter that is a concrete list, map, or set pins a bare argument. Later bindings in that for-comprehension use the pin. Other unbound kit parameters still match through `Type.eq`.
